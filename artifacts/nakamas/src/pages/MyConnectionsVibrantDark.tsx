...
export default function MyConnectionsVibrantDark() {
  const { t } = useLanguage();
  ...
  return (
    <div className="max-w-full mx-auto space-y-6 animate-in slide-in-from-bottom-4 duration-500 fade-in pb-20 md:pb-0">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <h1 className="text-xl md:text-2xl font-bold text-white">{t('Οι Συνδέσεις μου', 'My Nakamas')}</h1>
          <p className="text-white font-medium text-xs md:text-sm mt-1">{t('Άτομα με τα οποία συνδεθήκατε μέσα από εκδηλώσεις.', 'People you\'ve connected with through events.')}</p>
        </div>
      </div>

      <div className="flex gap-4 border-b border-gray-700">
        <button 
          className={`pb-3 text-sm font-bold border-b-2 transition-colors ${activeTab === 'all' ? 'border-fuchsia-600 text-fuchsia-400' : 'border-transparent text-white hover:text-white'}`}
          onClick={() => setActiveTab('all')}
        >
          {t('Συνδέσεις', 'Connections')} ({connections.length})
        </button>
        <button 
          className={`pb-3 text-sm font-bold border-b-2 transition-colors flex items-center gap-2 ${activeTab === 'requests' ? 'border-fuchsia-600 text-fuchsia-400' : 'border-transparent text-white hover:text-white'}`}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
        {connections.map((conn) => (
          <Card key={conn.id} className="p-4 flex items-center justify-between hover:border-fuchsia-800 transition-colors cursor-pointer group">
            ...
            <div className="flex items-center gap-2">
              <button className="p-2 text-fuchsia-400 hover:bg-fuchsia-900/30 rounded-full transition-colors" title={t('Μήνυμα', 'Message')} aria-label={t('Μήνυμα', 'Message')}>
                <MessageCircle className="w-5 h-5" />
              </button>
              <button className="p-2 text-gray-400 hover:bg-gray-700 rounded-full transition-colors" aria-label={t('Περισσότερες επιλογές', 'More options')} title={t('Περισσότερες επιλογές', 'More options')}>
