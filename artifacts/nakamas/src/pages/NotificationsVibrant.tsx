...
export default function NotificationsVibrant() {
    const { t } = useLanguage();
  ...
  return (
    <div className="mx-auto max-w-full space-y-6 pb-20 md:pb-0">
      <div className="flex justify-between items-end">
        ...
        <button className="text-[14.2457535px] font-bold text-fuchsia-600 hover:text-fuchsia-700">
      ...
      {todayNotifs.length > 0 && (
        <div className="space-y-3">
          ...
          {todayNotifs.map((notif) => {
            ...
            return (
              <div key={notif.id} className="flex gap-4 p-4 rounded-xl border border-fuchsia-100 bg-fuchsia-50/30">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {notif.type === 'match' && (
                    <button onClick={() => navigate('/plans')} className="mt-2 text-[11.2px] font-bold text-fuchsia-700 bg-fuchsia-100 px-2.5 py-1 rounded-full hover:bg-fuchsia-200 transition-colors inline-flex items-center gap-1">
                      {t(`Δείτε ομάδα`, `View group`)} <ArrowRight className="w-2.5 h-2.5" />
                    </button>
                  )}
                  {notif.type === 'message' && (
                    <button onClick={() => navigate('/chats')} className="mt-2 text-[11.2px] font-bold text-fuchsia-700 bg-fuchsia-100 px-2.5 py-1 rounded-full hover:bg-fuchsia-200 transition-colors inline-flex items-center gap-1">
                      {t(`Απάντηση`, `Reply`)} <ArrowRight className="w-2.5 h-2.5" />
                    </button>
                  )}
                </div>
                <span className="w-2.5 h-2.5 bg-fuchsia-500 rounded-full shrink-0 mt-2"></span>
              </div>
            );
          })}
        </div>
