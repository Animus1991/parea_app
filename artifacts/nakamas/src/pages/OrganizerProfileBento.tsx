...
export default function OrganizerProfileBento() {
  const { id } = useParams();
  const { t } = useLanguage();
        ...
        <div className="mt-6 sm:mt-0 flex-1 space-y-4">
          ...
          <div className="flex flex-wrap items-center justify-center sm:justify-start gap-4 pt-2">
            <button 
              ...
              className="text-[10px] font-bold tracking-wide text-black hover:text-indigo-600 transition-colors flex items-center gap-1"
            >
            ...
            {hostedEvents.map(event => (
              <Link key={event.id} to={`/events/${event.id}`}>
                <Card className="flex items-start gap-4 p-4 hover:border-indigo-300 transition-colors cursor-pointer group">
                  ...
                  <div className="hidden sm:flex items-center text-[10px] font-bold tracking-wide text-indigo-600 self-center">
                    {t('Προβολή', 'View')} &rarr;
                  </div>
                </Card>
              </Link>
