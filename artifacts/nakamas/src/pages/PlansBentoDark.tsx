...
export default function PlansBentoDark() {
  const { t } = useLanguage();
      ...
      <div className="space-y-4">
          ...
          <Card key={event.id} className="rounded-xl p-4 sm:p-5 border border-emerald-800 flex flex-col sm:flex-row gap-4">
            ...
            <div className="flex-1 flex flex-col">
              ...
              <div className="mt-auto pt-4 border-t border-gray-800 flex gap-2">
                ...
                <button onClick={() => navigate(`/events/${event.id}`)} className="bg-gray-800 border-gray-700 text-white border border-gray-700 hover:bg-gray-700/60 text-white px-4 py-2 rounded-lg text-[12.825px] font-bold transition-colors">
                  {t(`Λεπτομέρειες`, `Details`)}
                </button>
                <button className="bg-gray-800 border-gray-700 text-white border border-red-200 hover:bg-red-50 text-red-500 px-3 py-2 rounded-lg text-[12.825px] font-bold transition-colors flex items-center gap-1">
        ...
        {upcomingEvents.length === 0 && (
          <div className="text-center py-12 bg-gray-800/70 rounded-xl border border-dashed border-gray-700">
            ...
            <button className="bg-gray-800 shadow-[0_8px_30px_rgb(0,0,0,0.5)] border border-gray-700 text-white px-5 py-2 rounded-full text-[13.5375px] font-bold" onClick={() => navigate('/')}>{t(`Εξερεύνηση`, `Explore Experiences`)}</button>
          </div>
        )}
      </div>
      )}
      ...
      <div className="space-y-4">
        {pendingEvents.map(event => (
          <Card key={event.id} className="rounded-xl p-4 sm:p-5 border border-gray-700 flex flex-col sm:flex-row gap-4 opacity-75 hover:opacity-100 transition-opacity">
            <div className="w-full sm:w-32 h-32 sm:h-auto shrink-0 bg-gray-800 border border-gray-700 rounded-lg overflow-hidden relative grayscale">
              <img referrerPolicy="no-referrer" src={event.imageUrl} alt={event.title} className="w-full h-full object-cover" />
