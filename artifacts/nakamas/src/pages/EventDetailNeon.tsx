...
function Group({ group, event, navigate }: { group: any; event: any; navigate: any; key?: any }) {
  ...
  return (
    <div 
      className="group relative rounded-xl border border-gray-200 bg-white p-4 shadow-sm hover:border-emerald-300 hover:shadow-md transition-all cursor-pointer overflow-hidden mt-2" 
    ...
    >
              ...
              <div className="relative">
                <div className="h-8 w-8 rounded-full bg-gray-200 overflow-hidden shrink-0 border border-gray-200">
                   {groupHost.photoUrl ? (
                     ...
                     <div className="h-full w-full flex items-center justify-center bg-emerald-100 text-emerald-700 font-bold text-xs">
      ...
      <Button 
        variant="primary" 
        size="sm" 
        className="w-full bg-emerald-50 text-emerald-700 border border-emerald-100 hover:bg-emerald-600 hover:text-white transition-colors group-hover:bg-emerald-600 group-hover:text-white font-semibold shadow-sm"
...
export default function EventDetailNeon() {
      ...
      <div className="space-y-4">
        <div className="flex justify-between items-center">
          ...
          <div className="flex gap-2 flex-wrap justify-end">
            <button 
              onClick={handleSave}
              className={`flex items-center gap-2 text-[10px] font-bold  tracking-wider transition-colors px-3 py-1 rounded-full ${isSaved ? 'text-emerald-800 bg-emerald-100' : 'text-black hover:text-emerald-600 bg-gray-50 hover:bg-gray-100'}`}
            >
            ...
            <button 
              ...
              className="flex items-center gap-2 text-[10px] font-bold tracking-wide text-emerald-600 hover:text-emerald-800 transition-colors bg-emerald-50 px-3 py-1.5 rounded-full"
            >
            ...
            <button 
              onClick={handleShare}
              className="flex items-center gap-2 text-[10px] font-bold  tracking-wider text-emerald-600 hover:text-emerald-800 transition-colors bg-emerald-50 px-3 py-1.5 rounded-full"
            >
            ...
            <button 
              ...
              className="flex items-center gap-2 text-[10px] font-bold  tracking-wider text-black hover:text-emerald-600 transition-colors bg-gray-50 hover:bg-gray-100 px-3 py-1.5 rounded-full"
            >
        ...
        <div className="flex flex-wrap gap-2 mt-4">
          ...
          {event.minTrustTierAccess === '3_high_trust' && (
            <Badge variant="outline" className="text-emerald-700 font-bold bg-emerald-50 border-emerald-100" icon={<ShieldCheck className="h-3 w-3" />}>{t('Πρόσβαση με Επαλήθευση', 'Verified Access')}</Badge>
          )}
      ...
      <div className="grid grid-cols-1 gap-6 md:gap-8 md:grid-cols-5 lg:grid-cols-3">
          ...
          <section className="space-y-4 text-[13px] text-[#111827] leading-relaxed bg-white rounded-xl border border-gray-200 p-5 md:p-6 shadow-sm">
                ...
                <div className="space-y-3 col-span-2 sm:col-span-1">
                 ...
                 <div className="space-y-1 col-span-2 sm:col-span-1">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Link to="/trust" className="text-[11px] text-emerald-600 font-bold underline">{t('Γιατί;', 'Why?')}</Link>
                  </div>
                </div>
             </div>
             ...
             <div className={`${isMapFullscreen ? 'fixed !inset-0 !z-[9999] bg-black !m-0 rounded-none !h-[100dvh]' : 'mt-6 w-full h-64 sm:h-80 rounded-lg'} bg-gray-100 overflow-hidden relative border border-gray-200 transition-all duration-300`}>
                  <button 
                    ...
                    className="absolute top-4 right-4 z-10 p-2 bg-white/90 backdrop-blur rounded-lg shadow-md text-black hover:text-emerald-600 transition-colors"
                  >
             ...
             {organizer && (
               <div className="pt-5 border-t border-gray-200 mt-5">
                 ...
                 <div className="flex items-center justify-between p-4 rounded-xl border border-gray-200 bg-white shadow-sm hover:border-emerald-200 transition-colors">
                    <div className="flex items-center gap-4">
                      <div className="w-14 h-14 rounded-full overflow-hidden bg-emerald-50 border-2 border-white ring-2 ring-emerald-50 shrink-0">
                        {organizer.photoUrl ? (
                          <img referrerPolicy="no-referrer" src={organizer.photoUrl} alt={organizer.name} className="w-full h-full object-cover" />
                        ) : (
                          <div className="w-full h-full flex items-center justify-center text-emerald-500 font-bold text-lg">{organizer.name.substring(0, 2).toUpperCase()}</div>
                        )}
                      </div>
                      <div>
                        <div className="flex items-center gap-1.5 flex-wrap">
                          <Link to={`/profile`} className="text-base font-bold text-[#111827] hover:text-emerald-600 transition-colors">
                            {organizer.name}
                          </Link>
                          {organizer.trustTier && (
        ...
        <div className="space-y-6 md:col-span-2 lg:col-span-1">
          <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm sticky top-24">
            ...
            <div className="mb-4 bg-emerald-50 border border-emerald-100 rounded-lg p-3 flex flex-col xl:flex-row xl:justify-between items-start xl:items-center gap-2 text-sm font-bold text-emerald-900">
              ...
              <span className="text-emerald-600 xl:text-emerald-900 bg-white xl:bg-transparent px-2 py-0.5 xl:p-0 rounded-full text-xs xl:text-sm border border-emerald-100 xl:border-transparent">{Math.max(0, spotsLeftEvent)} {t('θέσεις έμειναν', 'spots left')}</span>
            </div>
                ...
                <div className="flex gap-2">
                  ...
                  <div className="flex flex-wrap gap-2 flex-1">
                    <select 
                      ...
                      className="text-xs bg-gray-50 border border-gray-200 rounded p-1 font-medium focus:outline-none focus:ring-1 focus:ring-emerald-500"
                    >
            ...
            <div className="space-y-4 pt-5 border-t border-gray-200">
              <div className="grid grid-cols-1 gap-3">
                <Button variant="outline" className="w-full border-gray-200 text-black hover:bg-gray-50" size="lg" onClick={() => toast.info(t("Προστέθηκε στη λίστα αναμονής. Θα ειδοποιηθείτε αν ανοίξει θέση.", "Added to waitlist. We will notify you if a spot in a group opens up."))}>
