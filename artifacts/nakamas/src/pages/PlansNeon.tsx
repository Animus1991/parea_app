...
export default function PlansNeon() {
  const { t } = useLanguage();
    ...
    <div className="mx-auto max-w-full space-y-6 md:space-y-8 pb-20 md:pb-0">
      ...
      <div className="space-y-4">
        {upcomingEvents.map(event => (
          <Card key={event.id} className="rounded-xl p-4 sm:p-5 border border-emerald-100 flex flex-col sm:flex-row gap-4">
            ...
            <div className="flex-1 flex flex-col">
              <div className="flex justify-between items-start gap-2 mb-2">
                ...
                <div className="text-right shrink-0">
                  ...
                  {(() => {
                    ...
                    if (days > 0 && days <= 7) return <span className="text-[10.639999999999999px] font-bold text-emerald-700 bg-emerald-50 px-1.5 py-0.5 rounded mt-0.5 inline-block">{t(`σε`, `in`)} {days} {t(`μέρες`, `days`)}</span>;
                    return null;
                  })()}
                </div>
              </div>
              ...
              <div className="mt-auto pt-4 border-t border-gray-100 flex gap-2">
                <button onClick={() => navigate(`/chat/${event.id}`)} className="flex-1 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 py-2 rounded-lg text-[12.825px] font-bold transition-colors flex items-center justify-center gap-1.5">
        ...
        {pastEvents.map((event, index) => (
          <Card key={event.id} className="rounded-xl p-4 sm:p-5 border border-gray-200 flex flex-col sm:flex-row gap-4">
            <div className="flex-1 flex flex-col">
              ...
              {index === 0 ? (
                <div className="bg-emerald-50 border border-emerald-100 p-3 rounded-lg flex items-center justify-between mt-auto">
                  <div className="text-[12.825px] font-bold text-emerald-900">{t(`Απαιτείται αξιολόγηση`, `Feedback required`)}</div>
