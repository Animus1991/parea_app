...
export default function EventDetailClassic() {
  ...
  const addToWaitlist = useStore((state) => state.addToWaitlist);
  const removeFromWaitlist = useStore((state) => state.removeFromWaitlist);
  const isWaitlisted = !!eventId && waitlistedEvents.includes(eventId);
        ...
        <div className="flex flex-wrap gap-2 mt-4">
          ...
          {currentUser && event.tags && event.tags.length > 0 && (
            <Badge variant="outline" className="bg-orange-50 border-orange-200 text-orange-700 font-bold" icon={<span className="text-[10px]">🔥</span>}>
              {Math.min(98, Math.max(15, Math.round(((currentUser.interests || []).filter(i => (event.tags ?? []).includes(i)).length / event.tags.length) * 100) + 40))}% {t('Ταίριασμα', 'Match')}
            </Badge>
          )}
        </div>
                 ...
                 <div className="space-y-1 col-span-2 sm:col-span-1">
                  ...
                  <div className="flex items-center gap-2">
                    <p className="font-medium text-[13px] capitalize">{
                      event.minTrustTierAccess === '3_high_trust'
                        ? t('απαιτείται επαληθευμένος λογαριασμός', 'verified account required')
                        : event.minTrustTierAccess === '2_confirmed'
                        ? t('απαιτείται επιβεβαιωμένος λογαριασμός', 'confirmed account required')
                        : t('διαθέσιμο σε όλους', 'open to all trust levels')
                    }</p>
          ...
          <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm sticky top-24">
            ...
            <div className="space-y-4 pt-5 border-t border-gray-200">
              <div className="grid grid-cols-1 gap-3">
                <Button
                  variant="outline"
                  className={`w-full border-gray-200 text-gray-700 hover:bg-gray-50 transition-colors ${isWaitlisted ? 'border-cyan-200 bg-cyan-50 text-cyan-700 hover:bg-cyan-100' : ''}`}
                  size="lg"
                  onClick={() => {
                    if (!eventId) return;
                    if (isWaitlisted) {
                ...
                >
                  {isWaitlisted
                    ? t('✓ Στη Λίστα Αναμονής', '✓ On Waitlist')
                    : t('Λίστα Αναμονής', 'Join Waitlist')}
                </Button>
              </div>
              <div className="text-[10px] text-gray-500 font-medium bg-gray-50 rounded text-center p-3 leading-relaxed tracking-wide">
