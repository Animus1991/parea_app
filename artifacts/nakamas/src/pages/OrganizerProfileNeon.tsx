...
export default function OrganizerProfileNeon() {
  const { id } = useParams();
  const { t } = useLanguage();
  ...
  if (!organizer) return <div className="p-8 text-center text-black font-medium">{t('Ο διοργανωτής δεν βρέθηκε', 'Organizer not found')}</div>;
        ...
        <div className="mt-6 sm:mt-0 flex-1 space-y-4">
          <div>
            <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2 mb-2">
              {isVerifiedOrganizer && (
                <Badge variant="outline" icon={<ShieldCheck className="h-3 w-3 text-emerald-600" />}>
            ...
            <p className="text-sm text-black font-medium mt-1"><MapPin className="h-3.5 w-3.5 inline mr-1" /> {organizer.city}</p>
          </div>
          ...
          <div className="flex items-center gap-4 bg-gray-50 p-3 rounded-lg border border-gray-100 w-fit">
             <div className="flex items-center gap-1 text-yellow-500">
               {[1, 2, 3, 4, 5].map(star => (
                 <svg key={star} className={`w-4 h-4 ${star <= 4 ? 'fill-current' : 'text-black'}`} viewBox="0 0 24 24">
             ...
             <div className="text-xs font-bold text-[#111827]">
               4.2 / 5.0 
               <span className="text-black font-medium ml-1">({t('120+ Κριτικές Εκδηλώσεων', '120+ Event Reviews')})</span>
             </div>
          </div>
          ...
          <div className="flex flex-wrap items-center justify-center sm:justify-start gap-4 pt-2">
            <button 
              ...
              className="text-[10px] font-bold tracking-wide text-black hover:text-emerald-600 transition-colors flex items-center gap-1"
            >
      ...
      <div className="space-y-4">
        ...
        {hostedEvents.length === 0 ? (
          <div className="text-center py-12 bg-gray-50 rounded-xl border border-dashed border-gray-200">
            <p className="text-black font-medium text-sm">{t('Δεν έχουν προγραμματιστεί επερχόμενες εκδηλώσεις.', 'No upcoming events currently scheduled.')}</p>
          </div>
        ) : (
              ...
              <Link key={event.id} to={`/events/${event.id}`}>
                <Card className="flex items-start gap-4 p-4 hover:border-emerald-300 transition-colors cursor-pointer group">
                  <div className="w-20 h-20 bg-gray-100 rounded-lg overflow-hidden shrink-0">
                    <div className="w-full h-full bg-emerald-50 flex flex-col items-center justify-center text-emerald-700">
                  ...
                  <div className="flex-1">
                    <h3 className="font-bold text-[#111827] group-hover:text-emerald-600 transition-colors line-clamp-1">{event.title}</h3>
                  ...
                  <div className="hidden sm:flex items-center text-[10px] font-bold tracking-wide text-emerald-600 self-center">
                    {t('Προβολή', 'View')} &rarr;
                  </div>
                </Card>
              </Link>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full p-6">
            <div className="flex justify-between items-center mb-4">
              ...
              <button onClick={() => setShowContactModal(false)} className="text-black hover:text-black transition-colors">
                <X className="h-5 w-5" />
              </button>
            </div>
