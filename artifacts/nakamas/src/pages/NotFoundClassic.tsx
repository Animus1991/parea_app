...
export default function NotFoundClassic() {
  const { t } = useLanguage();
  ...
  return (
    <div className="flex flex-col items-center justify-center py-16 px-6 text-center animate-in fade-in duration-500">
      <div className="w-20 h-20 rounded-full bg-gray-100 flex items-center justify-center mb-6 relative">
        <span className="absolute inset-0 rounded-full bg-gray-200 animate-ping opacity-20"></span>
        <Compass className="w-10 h-10 text-gray-400 animate-spin" style={{ animationDuration: '8s' }} />
      </div>
      <h1 className="text-[25px] font-bold text-[#111827] mb-2">
        {t(`Η σελίδα δεν βρέθηκε`, `Page Not Found`)}
      </h1>
      <p className="text-[18px] text-gray-500 font-medium mb-6 max-w-xs">
