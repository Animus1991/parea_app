...
export default function HomeClassic() {
  ...
  // Auto-request location when user activates the radius filter
  useEffect(() => {
    if (radiusFilter !== "Any" && !geo.granted && !geo.loading && !geo.error) {
      geo.request();
    }
  }, [radiusFilter]);

  const getDistance = useMemo(() => {
    return (ev: (typeof events)[0]): number => {
      if (geo.granted && geo.lat !== null && geo.lng !== null && ev.lat && ev.lng) {
        return haversineDistance(geo.lat, geo.lng, ev.lat, ev.lng);
      }
      return mockDistances[ev.id] ?? 5;
    };
