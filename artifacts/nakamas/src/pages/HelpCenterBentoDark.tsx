...
export default function HelpCenterBentoDark() {
    const { t } = useLanguage();
  ...
  return (
    <div className="max-w-full mx-auto space-y-6 animate-in slide-in-from-bottom-4 duration-500 fade-in pb-20 md:pb-0">
      <div className="bg-emerald-600 text-white rounded-2xl p-8 text-center shadow-md relative overflow-hidden">
        <div className="absolute top-0 right-0 -mr-8 -mt-8 w-32 h-32 rounded-full bg-gray-800 border-gray-700 text-white opacity-10"></div>
        <div className="absolute bottom-0 left-0 -ml-8 -mb-8 w-24 h-24 rounded-full bg-gray-800 border-gray-700 text-white opacity-10"></div>
        ...
        <p className="text-emerald-900 text-[16.2px] font-medium mb-6 relative z-10">{t(`Πώς μπορούμε να σας βοηθήσουμε;`, `How can we help you?`)}</p>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {popularTopics.map(topic => {
          ...
          return (
            <button key={topic.label} className={`flex items-center gap-2 p-3 rounded-xl border border-gray-800 bg-gray-800 border-gray-700 text-white hover:shadow-sm transition-all`}>
      ...
      <div>
        ...
        <div className="space-y-3">
          {filtered.map(article => (
            <Card key={article.id} className="p-4 flex items-center justify-between hover:border-emerald-800 cursor-pointer transition-colors">
              <div>
                <span className="text-[12.5px] font-bold text-emerald-400 tracking-wide">{article.category}</span>
