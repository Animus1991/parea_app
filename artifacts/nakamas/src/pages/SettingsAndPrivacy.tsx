  ...
  const sections = [
    ...
    },
    {
      ...
      items: [
        ...
        { icon: Lock, label: 'Two-Factor Authentication', value: twoFactorEnabled ? 'On' : 'Off', onClick: () => setShow2FASetup(!show2FASetup) },
      ]
    },
    {
      title: 'Notifications',
      items: [
        { icon: Bell, label: 'Notification Preferences', value: 'Custom', onClick: () => setShowNotifications(!showNotifications) },
      ]
    }
  ];
                ...
                <div key={itemIdx}>
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2 text-xs text-gray-500">
                      {item.value}
                      <svg className={`w-4 h-4 text-gray-400 transition-transform ${item.onClick && (item.label === 'Two-Factor Authentication' && show2FASetup || item.label === 'Notification Preferences' && showNotifications) ? 'rotate-90' : 'group-hover:text-indigo-600'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  ...
                  {item.label === 'Two-Factor Authentication' && show2FASetup && (
                    <div className="p-5 bg-gray-50/50 border-t border-gray-100 animate-in slide-in-from-top-2">
                      {!twoFactorEnabled ? (
                        <div className="space-y-4">
                          <p className="text-sm text-gray-600">Protect your account using an authenticator app or SMS text messages.</p>
                          <div className="grid gap-3 sm:grid-cols-2">
                            <Card className="p-4 border-indigo-100 hover:border-indigo-300 cursor-pointer" onClick={() => setTwoFactorEnabled(true)}>
                              ...
                              <h4 className="font-bold text-sm text-gray-900 mb-1">Authenticator App</h4>
                              <p className="text-xs text-gray-500">Use apps like Google Authenticator or Authy (Recommended).</p>
                            </Card>
                            <Card className="p-4 border-gray-200 hover:border-gray-300 cursor-pointer" onClick={() => setTwoFactorEnabled(true)}>
                              ...
                              <h4 className="font-bold text-sm text-gray-900 mb-1">Text Message (SMS)</h4>
                              <p className="text-xs text-gray-500">Receive a code via SMS to your verified phone number.</p>
                            </Card>
                          </div>
                        </div>
                      ) : (
                        <div className="space-y-4">
                          <div className="flex items-start gap-3 bg-emerald-50 p-3 rounded-lg border border-emerald-100">
                            ...
                            <div>
                              <h4 className="font-bold text-sm text-emerald-900 tracking-tight">Two-Factor Authentication is On</h4>
                              <p className="text-xs text-emerald-700 mt-0.5">Your account is secured. We'll ask for a code when you sign in from unrecognized devices.</p>
                            </div>
                          </div>
                  ...
                  {item.label === 'Notification Preferences' && showNotifications && (
                    <div className="p-5 bg-gray-50/50 border-t border-gray-100 animate-in slide-in-from-top-2 space-y-6">
                      
                      <div className="space-y-4">
                        <h4 className="text-xs font-bold text-gray-500 uppercase flex items-center gap-2"><Bell className="w-3.5 h-3.5" /> Push Notifications</h4>
                        <div className="space-y-3 pl-1">
                          <label className="flex items-center justify-between cursor-pointer">
                            <span className="text-sm font-medium text-gray-700">Direct Messages & Group Chats</span>
                          ...
                          <label className="flex items-center justify-between cursor-pointer">
                            <span className="text-sm font-medium text-gray-700">Event Reminders & Updates</span>
                      ...
                      <div className="space-y-4 pt-4 border-t border-gray-200">
                        <h4 className="text-xs font-bold text-gray-500 uppercase flex items-center gap-2"><Mail className="w-3.5 h-3.5" /> Email Notifications</h4>
                        <div className="space-y-3 pl-1">
                          <label className="flex items-center justify-between cursor-pointer">
                            <span className="text-sm font-medium text-gray-700">Important Account Updates</span>
                          ...
                          <label className="flex items-center justify-between cursor-pointer">
                            <span className="text-sm font-medium text-gray-700">Marketing & Promotional</span>
