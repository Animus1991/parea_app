...
export default function AchievementsBento() {
  ...
  const achievements: Achievement[] = [
    { id: 'a1', icon: Calendar, title: t(`Πρώτη Εμπειρία`, `First Experience`), description: t(`Συμμετείχατε στην πρώτη σας εκδήλωση`, `Attended your first event`), progress: 1, maxProgress: 1, unlocked: true, xp: 50, color: 'text-indigo-600 bg-indigo-100' },
      ...
      <Card className="p-5">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-gradient-to-br from-indigo-500 to-purple-500 flex items-center justify-center text-white font-black text-[18px]">
              {level}
            </div>
            <div>
        ...
        <div className="w-full bg-gray-100 h-2 rounded-full overflow-hidden">
          <div className="bg-gradient-to-r from-indigo-500 to-purple-500 h-full rounded-full transition-all" style={{ width: `${levelProgress}%` }} />
        </div>
      </Card>

      {/* Daily Challenge */}
      <Card className="p-4 bg-gradient-to-r from-purple-50 to-indigo-50 border border-purple-100">
        ...
        {[
          ...
          <button
            ...
            className={`pb-3 text-[13.5px] font-bold tracking-wide transition-colors whitespace-nowrap ${
              activeTab === tab.key ? 'border-b-2 border-indigo-600 text-indigo-900' : 'text-black hover:text-[#111827]'
            }`}
          >
            {tab.label}
          </button>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      <div className="flex-1 bg-gray-100 h-1.5 rounded-full overflow-hidden">
                        <div className={`h-full rounded-full transition-all ${a.unlocked ? 'bg-indigo-500' : 'bg-gray-300'}`} style={{ width: `${pct}%` }} />
                      </div>
                      <span className="text-[11.2px] font-bold text-black">{a.progress}/{a.maxProgress}</span>
                    </div>
                  </div>
                  <div className="flex flex-col items-end gap-1 shrink-0">
                    <span className="text-[11.2px] font-bold text-indigo-600 bg-indigo-50 px-1.5 py-0.5 rounded">+{a.xp}XP</span>
                    {a.unlocked && (
                      <button className="text-[10px] font-bold text-black hover:text-indigo-600 flex items-center gap-0.5 transition-colors">
      ...
      {activeTab === 'leaderboard' && (
        <Card className="p-0 overflow-hidden">
          {leaderboard.map((entry, i) => (
            <div key={entry.rank} className={`flex items-center gap-3 px-4 py-3 border-b border-gray-50 last:border-0 ${entry.rank === 4 ? 'bg-indigo-50/50' : ''}`}>
              ...
              <span className={`flex-1 text-[13.5px] font-medium ${entry.rank === 4 ? 'font-bold text-indigo-700' : 'text-[#111827]'}`}>{entry.name}</span>
