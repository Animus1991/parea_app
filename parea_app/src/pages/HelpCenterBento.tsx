...
export default function HelpCenterBento() {
    const { t } = useLanguage();
  ...
  return (
    <div className="max-w-full mx-auto space-y-6 animate-in slide-in-from-bottom-4 duration-500 fade-in pb-20 md:pb-0">
      <div className="bg-indigo-600 text-white rounded-2xl p-8 text-center shadow-md relative overflow-hidden">
        ...
        <p className="text-indigo-100 text-[16.2px] font-medium mb-6 relative z-10">{t(`Πώς μπορούμε να σας βοηθήσουμε;`, `How can we help you?`)}</p>
      ...
      <div>
        ...
        <div className="space-y-3">
          {filtered.map(article => (
            <Card key={article.id} className="p-4 flex items-center justify-between hover:border-indigo-200 cursor-pointer transition-colors">
              <div>
                <span className="text-[12.5px] font-bold text-indigo-600 tracking-wide">{article.category}</span>
      ...
      <Card className="p-6 text-center">
        ...
        <button className="px-4 py-2 bg-indigo-600 text-white text-[13.5px] font-bold rounded-lg hover:bg-indigo-700 transition-colors">
