...
export default function SettingsAndPrivacyVibrantDark() {
    const { t } = useLanguage();
  ...
  return (
    <div className="max-w-full mx-auto space-y-6 animate-in slide-in-from-bottom-4 duration-500 fade-in pb-20 md:pb-0">
      <div className="mb-2">
        <h1 className="text-[20.104264919475px] md:text-[26.7902365993px] font-bold text-white">{t(`Ρυθμίσεις & Απόρρητο`, `Settings & Privacy`)}</h1>
        <p className="text-white font-medium text-[13.55510121105px] md:text-[14.626916949961px] mt-1">{t(`Διαχειριστείτε τον λογαριασμό σας`, `Manage your account`)}</p>
      </div>

      {sections.map((section, idx) => (
        <div key={idx}>
          <h2 className="text-[12.1125px] font-bold tracking-wider text-white mb-3">{section.title}</h2>
            ...
            {section.items.map((item, i) => {
              ...
              return (
                <div key={i} onClick={item.onClick} className={`flex items-center justify-between p-4 ${item.onClick ? 'cursor-pointer hover:bg-gray-700/60' : ''} transition-colors`}>
                  ...
                  <span className="text-[13.5px] text-white font-medium">{item.value}</span>
                </div>
              );
            })}
          </Card>
      ...
      {show2FASetup && (
        <Card className="p-5 border-fuchsia-800 bg-fuchsia-900/30/30">
      ...
      {showNotifications && (
        <div className="space-y-4">
          <Card className="p-5">
            <h3 className="font-bold text-[16.2px] text-white mb-4">{t(`Γενικές Ειδοποιήσεις`, `General Notifications`)}</h3>
            <div className="space-y-3">
              {[
                ...
                <label key={key} className="flex items-center justify-between">
                  ...
                  <div className={`relative w-9 h-5 rounded-full transition-colors cursor-pointer ${notificationPrefs[key as keyof typeof notificationPrefs] ? 'bg-fuchsia-600' : 'bg-gray-300'}`} onClick={() => handleTogglePref(key as keyof typeof notificationPrefs)}>
                    <div className={`absolute top-0.5 w-4 h-4 rounded-full bg-gray-800 shadow transition-transform ${notificationPrefs[key as keyof typeof notificationPrefs] ? 'translate-x-4' : 'translate-x-0.5'}`} />
                  </div>
                </label>
              ))}
            </div>
          ...
          <Card className="p-5">
            <h3 className="font-bold text-[16.2px] text-white mb-1">{t(`Κατηγορίες Ενδιαφέροντος`, `Category Subscriptions`)}</h3>
            <p className="text-[12.5px] text-white font-medium mb-4">{t(`Ειδοποίηση μόνο για εκδηλώσεις στις κατηγορίες που σας ενδιαφέρουν`, `Get notified only for events in categories you care about`)}</p>
      ...
      {/* Active Sessions */}
      <div>
        <h2 className="text-[13.5px] font-bold tracking-wider text-white mb-3">{t(`Ενεργές Συνεδρίες`, `Active Sessions`)}</h2>
        <Card className="divide-y divide-gray-100">
          <div className="flex items-center justify-between p-4">
            <div className="flex items-center gap-3">
              <Smartphone className="w-4 h-4 text-fuchsia-400" />
              <div>
                <span className="text-[16.2px] font-medium text-white">iPhone 15 Pro</span>
                <p className="text-[12.5px] text-white font-medium">Athens, GR • {t(`Τώρα`, `Now`)}</p>
              </div>
            </div>
            <span className="text-[11.2px] font-bold text--400 bg-fuchsia-950 px-2 py-0.5 rounded-full">{t(`Τρέχουσα`, `Current`)}</span>
          </div>
          <div className="flex items-center justify-between p-4">
            <div className="flex items-center gap-3">
              <Laptop className="w-4 h-4 text-white" />
              <div>
                <span className="text-[16.2px] font-medium text-white">Chrome • Windows</span>
                <p className="text-[12.5px] text-white font-medium">Athens, GR • 2h {t(`πριν`, `ago`)}</p>
              </div>
            </div>
            <button className="text-[11.2px] font-bold text-red-500 hover:text--400">{t(`Αποσύνδεση`, `Log out`)}</button>
          </div>
        </Card>
      </div>
      ...
      {/* Data Export */}
      <div>
        <h2 className="text-[13.5px] font-bold tracking-wider text-white mb-3">{t(`Δεδομένα`, `Data`)}</h2>
        <Card className="p-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <Download className="w-4 h-4 text-white" />
              <div>
                <span className="text-[16.2px] font-medium text-white">{t(`Εξαγωγή δεδομένων`, `Export your data`)}</span>
                <p className="text-[12.5px] text-white font-medium">{t(`Κατεβάστε αντίγραφο των δεδομένων σας`, `Download a copy of your data`)}</p>
              </div>
            </div>
      ...
      <div className="pt-4 space-y-3">
        <Button variant="outline" className="w-full text--400 border-red-200 hover:bg-red-50">
          <LogOut className="w-4 h-4 mr-2" /> {t(`Αποσύνδεση`, `Log out`)}
        </Button>
        <button className="w-full text-[13.5px] text-red-400 font-medium hover:text--400 transition-colors py-2">
      ...
      <div className="text-center pt-2 pb-4">
        <div className="flex items-center justify-center gap-1 text-[12.5px] text-white font-medium">
