...
export default function SavedEventsVibrantDark() {
  const { t } = useLanguage();
      ...
      {savedEvents.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {savedEvents.map(event => (
            <Link key={event.id} to={`/events/${event.id}`} className="bg-gray-800 rounded-xl border border-gray-700 overflow-hidden shadow-sm hover:shadow-md transition-all group flex flex-col">
              <div className="h-32 bg-gray-200 relative overflow-hidden shrink-0">
                ...
                <button className="absolute top-3 right-3 w-8 h-8 rounded-full bg-gray-800 flex items-center justify-center text-white shadow-sm z-10" aria-label={t('Αποθηκευμένο', 'Saved')}>
                  <Bookmark className="w-4 h-4 fill-current text-fuchsia-400" />
                </button>
              </div>
              <div className="p-4 flex flex-col flex-1">
                ...
                <h3 className="font-bold text-sm text-white group-hover:text-fuchsia-400 transition-colors line-clamp-2 mb-2">{event.title}</h3>
                ...
                <div className="mt-4 pt-3 border-t border-gray-800 flex items-center gap-1.5 text-[10px] font-bold text-fuchsia-400 tracking-wider">
        ...
        <div className="text-center py-16 bg-gray-800/70 rounded-2xl border border-dashed border-gray-700">
          ...
          <Link to="/" className="inline-flex items-center justify-center px-4 py-2 mt-4 bg-fuchsia-600 text-white text-xs font-bold rounded-full shadow-sm hover:bg-fuchsia-700 transition-colors">
