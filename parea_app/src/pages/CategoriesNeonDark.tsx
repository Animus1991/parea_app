...
export default function CategoriesNeonDark() {
  const navigate = useNavigate();
  const { t } = useLanguage();
  const events = useStore((state) => state.events);

  const ALL_CATEGORIES = [
    { id: '1', name: t('Θεατρο', 'Theatre'), icon: Ticket, count: 12, color: 'text--400', bg: 'bg-teal-950', border: 'border-teal-900' },
    { id: '2', name: t('Συναυλιες', 'Concerts'), icon: Music, count: 8, color: 'text--400', bg: 'bg-purple-50', border: 'border-purple-100' },
    { id: '3', name: t('Σινεμα', 'Cinema'), icon: Camera, count: 15, color: 'text-emerald-400', bg: 'bg-emerald-900/30', border: 'border-emerald-800' },
    ...
    { id: '10', name: t('Ανταλλαγη γλωσσων', 'Language exchange'), icon: Languages, count: 14, color: 'text-emerald-400', bg: 'bg-emerald-900/30', border: 'border-emerald-800' },
    ...
    { id: '17', name: t('Ελαφρια αθληματα', 'Light sports'), icon: Trophy, count: 19, color: 'text--400', bg: 'bg-emerald-950', border: 'border-emerald-900' },
    ...
    { id: '20', name: t('Κοινοτικες εκδηλωσεις', 'Community events'), icon: HeartHandshake, count: 28, color: 'text-teal-9500', bg: 'bg-teal-950', border: 'border-teal-900' },
    ...
    <div className="max-w-full mx-auto space-y-6 animate-in slide-in-from-bottom-4 duration-500 fade-in pb-20 md:pb-0">
      ...
      <div className="relative mb-6 max-w-md">
        ...
        <input 
          ...
          className="w-full h-11 pl-10 pr-4 rounded-full border border-gray-700 bg-gray-800 shadow-sm focus:outline-none focus:ring-2 focus:ring-emerald-600 focus:border-transparent text-sm font-medium transition-shadow hover:shadow-md"
        />
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 xl:grid-cols-6 gap-3">
        {visibleCategories.map(cat => (
          <div 
            key={cat.id} 
            className={`bg-gray-800 rounded-xl p-3 border shadow-sm hover:shadow-md transition-all cursor-pointer group flex flex-col items-center text-center ${activeCategory === cat.name ? 'border-emerald-600 ring-1 ring-emerald-600' : 'border-gray-800 hover:border-gray-700'}`}
          ...
          >
            ...
            <h3 className="font-bold text-white text-xs group-hover:text-emerald-400 transition-colors line-clamp-1 w-full">{cat.name}</h3>
            ...
            <div className="flex gap-1 shrink-0 bg-gray-700/80 p-0.5 rounded-lg border border-gray-700/50">
              {['All', 'Free', 'Paid', 'Group Discount'].map((price) => (
                <button
                  ...
                  className={`text-[10px] sm:text-xs font-semibold px-2.5 sm:px-3 py-1.5 rounded-md transition-all whitespace-nowrap ${priceFilter === price ? 'bg-gray-800 text-emerald-400 shadow-sm border border-gray-700/50 ring-1 ring-black/5' : 'text-white hover:text-white hover:bg-gray-200/50'}`}
                >
            ...
            <div className="flex gap-1 shrink-0 bg-gray-700/80 p-0.5 rounded-lg border border-gray-700/50">
              {['Any', 'Today', 'This Week', 'This Month'].map((date) => (
                <button
                  ...
                  className={`text-[10px] sm:text-xs font-semibold px-2.5 sm:px-3 py-1.5 rounded-md transition-all whitespace-nowrap ${dateFilter === date ? 'bg-gray-800 text-emerald-400 shadow-sm border border-gray-700/50 ring-1 ring-black/5' : 'text-white hover:text-white hover:bg-gray-200/50'}`}
                >
