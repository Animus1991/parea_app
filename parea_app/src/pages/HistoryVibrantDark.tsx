...
export default function HistoryVibrantDark() {
    const { t } = useLanguage();
  ...
  return (
    <div className="max-w-full mx-auto space-y-6 animate-in slide-in-from-bottom-4 duration-500 fade-in pb-20 md:pb-0">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <h1 className="text-[20.104264919475px] md:text-[26.7902365993px] font-bold text-white">{t(`Ιστορικό`, `History`)}</h1>
          <p className="text-white font-medium text-[13.551608211075px] md:text-[14.626916949961px] mt-1">{t(`Εκδηλώσεις που έχετε παρακολουθήσει`, `Events you've attended`)}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        <Card className="p-3 text-center">
          ...
          <p className="text-[11.2px] text-white font-medium tracking-wide">{t(`Εκδηλώσεις`, `Events`)}</p>
        </Card>
        <Card className="p-3 text-center">
          <p className="text-[23px] font-black text-white">4.5</p>
          <p className="text-[11.2px] text-white font-medium tracking-wide">{t(`Μ.Ο. Βαθμολογία`, `Avg Rating`)}</p>
        </Card>
        <Card className="p-3 text-center">
          <p className="text-[23px] font-black text-white">14</p>
          <p className="text-[11.2px] text-white font-medium tracking-wide">{t(`Άτομα`, `People Met`)}</p>
        </Card>
      </div>

      <div className="space-y-3">
        {pastEvents.map((event) => (
          <Card key={event.id} className="p-4 hover:border-fuchsia-800 transition-colors cursor-pointer" onClick={() => navigate(`/history/feedback/${event.id}`)}>
            <div className="flex gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <h3 className="font-bold text-[16.75971px] text-white truncate">{event.title}</h3>
                <div className="flex flex-wrap items-center gap-3 mt-1.5 text-[12.1125px] text-white font-medium">
              ...
              <div className="self-center shrink-0">
                {feedbackStatus[event.id]?.given ? (
                  <span className="flex items-center gap-1 text-[11.2px] font-bold text--400 bg-green-50 px-2 py-1 rounded-full">
