...
export default function PostEventFeedbackBento() {
    const { t } = useLanguage();
          ...
          <div className="space-y-8">
              ...
              <div className="flex gap-3 justify-center">
                {moods.map(m => (
                  <button
                    ...
                    className={`flex flex-col items-center gap-1 p-3 rounded-xl border transition-all ${mood === m.emoji ? 'border-indigo-500 bg-indigo-50 scale-110 shadow-sm' : 'border-gray-200 hover:border-gray-300'}`}
                  >
              ...
              <div className="flex gap-2">
                {[1, 2, 3, 4, 5].map((star) => (
                  <button key={`vibe-${star}`} onClick={() => setVibeRating(star)} className="p-2 hover:bg-gray-50 rounded-full transition-colors">
                    <Star className={`w-8 h-8 transition-colors ${star <= vibeRating ? 'text-indigo-400 fill-indigo-400' : 'text-black hover:text-indigo-400'}`} />
                  </button>
                ))}
              </div>
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-[16.2px] font-medium focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-none"
                rows={3}
              />
