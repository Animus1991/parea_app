...
export default function NotFoundVibrant() {
  const { t } = useLanguage();
  ...
  return (
    <div className="flex flex-col items-center justify-center py-16 px-6 text-center animate-in fade-in duration-500">
      <div className="w-20 h-20 rounded-full bg-gradient-to-br from-fuchsia-100 to-orange-100 flex items-center justify-center mb-6 relative">
        <span className="absolute inset-0 rounded-full bg-fuchsia-200 animate-ping opacity-20"></span>
        <Compass className="w-10 h-10 text-fuchsia-600 animate-spin" style={{ animationDuration: '8s' }} />
      </div>
      <h1 className="text-[25px] font-bold text-[#111827] mb-2">
        {t(`Η σελίδα δεν βρέθηκε`, `Page Not Found`)}
      </h1>
      <p className="text-[18px] text-black font-medium mb-6 max-w-xs">
      ...
      <div className="grid grid-cols-2 gap-2 mb-6 w-full max-w-xs">
        {suggestions.map(s => {
          const Icon = s.icon;
          return (
            <button key={s.path} onClick={() => navigate(s.path)} className="flex items-center gap-2 p-3 rounded-xl border border-gray-200 bg-white hover:border-fuchsia-300 hover:bg-fuchsia-50 transition-all text-left">
