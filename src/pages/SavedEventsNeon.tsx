...
export default function SavedEventsNeon() {
  const { t } = useLanguage();
            ...
            <Link key={event.id} to={`/events/${event.id}`} className="bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm hover:shadow-md transition-all group flex flex-col">
              <div className="h-32 bg-gray-200 relative overflow-hidden shrink-0">
                ...
                <button className="absolute top-3 right-3 w-8 h-8 rounded-full bg-white flex items-center justify-center text-[#111827] shadow-sm z-10">
                  <Bookmark className="w-4 h-4 fill-current text-emerald-600" />
                </button>
              </div>
              <div className="p-4 flex flex-col flex-1">
                ...
                <h3 className="font-bold text-sm text-[#111827] group-hover:text-emerald-600 transition-colors line-clamp-2 mb-2">{event.title}</h3>
                ...
                <div className="mt-4 pt-3 border-t border-gray-100 flex items-center gap-1.5 text-[10px] font-bold text-emerald-600 tracking-wider">
