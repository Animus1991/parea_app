...
function Group({ group, event, navigate }: { group: any; event: any; navigate: any; key?: any }) {
  ...
  return (
    <div 
      className="group relative rounded-xl border border-gray-700 bg-gray-800 border-gray-700 text-white p-4 shadow-sm hover:border-cyan-300 hover:shadow-md transition-all cursor-pointer overflow-hidden mt-2" 
    ...
    >
      ...
      <Button 
        variant="primary" 
        size="sm" 
        className="w-full bg-emerald-900/30 text-emerald-400 border border-emerald-800 hover:bg-cyan-600 hover:text-white transition-colors group-hover:bg-cyan-600 group-hover:text-white font-semibold shadow-sm"
...
export default function EventDetailBentoDark() {
    ...
    >
      ...
      <div className="space-y-4">
        <div className="flex justify-between items-center">
          ...
          <div className="flex gap-2 flex-wrap justify-end">
            <button 
              onClick={handleSave}
              className={`flex items-center gap-2 text-[10px] font-bold  tracking-wider transition-colors px-3 py-1 rounded-full ${isSaved ? 'text-emerald-400 bg-emerald-900/50' : 'text-white hover:text-emerald-400 bg-gray-900 hover:bg-gray-800 border border-gray-700'}`}
            >
            ...
            <button 
              ...
              className="flex items-center gap-2 text-[10px] font-bold  tracking-wider text-white hover:text-emerald-400 transition-colors bg-gray-900 hover:bg-gray-800 border border-gray-700 px-3 py-1.5 rounded-full"
            >
      ...
      <div className="grid grid-cols-1 gap-6 md:gap-8 md:grid-cols-5 lg:grid-cols-3">
        ...
        <div className="space-y-6 md:space-y-8 md:col-span-3 lg:col-span-2">
          <section className="space-y-4 text-[13px] text-white leading-relaxed bg-gray-800 border-gray-700 text-white rounded-xl border border-gray-700 p-5 md:p-6 shadow-sm">
             ...
             <div className={`${isMapFullscreen ? 'fixed !inset-0 !z-[9999] bg-black !m-0 rounded-none !h-[100dvh]' : 'mt-6 w-full h-64 sm:h-80 rounded-lg'} bg-gray-800 border border-gray-700 overflow-hidden relative border border-gray-700 transition-all duration-300`}>
                  <button 
                    onClick={() => setIsMapFullscreen(!isMapFullscreen)}
                    className="absolute top-4 right-4 z-10 p-2 bg-gray-800 border-gray-700 text-white/90 backdrop-blur rounded-lg shadow-md text-white hover:text-emerald-400 transition-colors"
                  >
                  ...
                  {apiKey ? (
                    <ErrorBoundary fallback={
                      <div className="w-full h-full flex items-center justify-center bg-[#e5e3df] p-4 text-center">
                        <div className="bg-gray-800 border-gray-700 text-white p-3 rounded-lg shadow-sm border border-red-100">
                    ...
                    <div className="w-full h-full bg-[#e5e3df] relative flex items-center justify-center overflow-hidden">
                       ...
                       <div className="absolute bottom-2 right-2 bg-gray-800 border-gray-700 text-white/90 backdrop-blur text-white text-[10px] px-2 py-1 rounded shadow-sm">{t('Ενεργή Προεπισκόπηση Χάρτη', 'Map Preview Active')}</div>
                    </div>
                  )}
             </div>
             ...
             <div className="pt-5 border-t border-gray-700 mt-5">
               ...
               {event.externalLink && (
                 <div className="mt-4 pt-1">
                   <a href={event.externalLink} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-4 py-2 border border-gray-700 text-white bg-gray-800 border-gray-700 text-white hover:bg-gray-900 hover:text-emerald-400 hover:border-cyan-200 transition-colors shadow-sm text-xs font-bold tracking-wide rounded-lg w-full sm:w-auto justify-center">
               ...
               {event.tags && event.tags.length > 0 && (
                 <div className="flex flex-wrap gap-2 mt-4 pt-4 border-t border-gray-800">
                   {event.tags.map((tag: string) => (
                     <Badge key={tag} variant="neutral" className="bg-gray-800 border border-gray-700 text-white hover:bg-gray-200 shadow-none border border-gray-700/60 px-3 py-1 text-xs cursor-pointer transition-colors" onClick={() => navigate(`/?search=${tag}`)}>
             ...
             {organizer && (
               <div className="pt-5 border-t border-gray-700 mt-5">
                 ...
                 <div className="flex items-center justify-between p-4 rounded-xl border border-gray-700 bg-gray-800 border-gray-700 text-white shadow-sm hover:border-cyan-200 transition-colors">
                  ...
                  <div className="flex flex-wrap gap-2 flex-1">
                    ...
                    {event.isPaid && event.groupDiscount && (
                      <button 
                        ...
                        className={`text-xs px-2 py-1 rounded border font-medium transition-colors ${discountFilter ? 'bg-emerald-100 border-emerald-300 text--400' : 'bg-gray-900 border-gray-700 text-white hover:bg-gray-800 border border-gray-700'}`}
                      >
      ...
      {/* Mobile Sticky CTA */}
      <div className="fixed bottom-0 left-0 right-0 bg-gray-800 border-gray-700 text-white border-t border-gray-700 p-4 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] md:hidden z-50 flex gap-3">
      ...
      {showQRCode && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm px-4" onClick={() => setShowQRCode(false)}>
          <div className="bg-gray-800 border-gray-700 text-white rounded-2xl p-6 md:p-8 max-w-sm w-full text-center relative shadow-2xl" onClick={e => e.stopPropagation()}>
            <button 
              onClick={() => setShowQRCode(false)}
              className="absolute top-4 right-4 p-2 text-white hover:text-white bg-gray-800 border border-gray-700 hover:bg-gray-200 rounded-full transition-colors"
            >
              <X className="h-4 w-4" />
            </button>
            ...
            <div className="bg-gray-800 border-gray-700 text-white p-4 rounded-xl shadow-inner border border-gray-800 inline-block">
              <QRCodeSVG 
