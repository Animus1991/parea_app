...
export default function TrustCenterVibrantDark() {
    const { t } = useLanguage();
  return (
    <div className="mx-auto max-w-full space-y-6 md:space-y-8">
      <div>
        <h1 className="text-[20.104264919475px] md:text-[26.7902365993px] font-bold text-white tracking-tight">{t(`Κέντρο Εμπιστοσύνης`, `Trust Center`)}</h1>
        <p className="mt-2 text-[16.2px] leading-relaxed text-white max-w-xl">{t(`Η αξιοπιστία σας βασίζεται στις επαληθεύσεις και τις αξιολογήσεις σας.`, `Your trust score is based on your verifications and reviews.`)}</p>
      </div>
           ...
           <div className="space-y-3">
             <div className="flex items-center justify-between">
               <div className="flex items-center gap-2 text-[13.5px] text-white font-medium">
                 <Mail className="h-3.5 w-3.5 text-white" /> Email
               </div>
               <Badge variant="success" className="text-[11.2px] px-1.5 py-0.5">{t(`Επαληθευμένο`, `Verified`)}</Badge>
             </div>
             <div className="flex items-center justify-between">
               <div className="flex items-center gap-2 text-[13.5px] text-white font-medium">
                 <Phone className="h-3.5 w-3.5 text-white" />{t(`Τηλέφωνο`, `Phone`)}</div>
               <Badge variant="success" className="text-[11.2px] px-1.5 py-0.5">{t(`Επαληθευμένο`, `Verified`)}</Badge>
             </div>
             <div className="flex items-center justify-between">
               <div className="flex items-center gap-2 text-[13.5px] text-white font-medium">
                 <CreditCard className="h-3.5 w-3.5 text-white" />{t(`Πληρωμή`, `Payment`)}</div>
               <Badge variant="success" className="text-[11.2px] px-1.5 py-0.5">{t(`Επαληθευμένο`, `Verified`)}</Badge>
             </div>
             <div className="flex items-center justify-between">
               <div className="flex items-center gap-2 text-[13.5px] text-white font-medium">
                 <UserCheck className="h-3.5 w-3.5 text-white" />{t(`Ταυτότητα`, `Government ID`)}</div>
               <span className="text-[11.2px] text-fuchsia-400 font-bold cursor-pointer hover:underline tracking-wide">{t(`Επαλήθευση`, `Verify`)}</span>
             </div>
           </div>
           
           <div className="pt-4 border-t border-gray-800">
             <div className="flex items-center justify-between">
               <div className="flex items-center gap-2 text-[13.8px] font-bold text-white">
                 <Award className="h-4 w-4 text-amber-500" />{t(`Βαθμός Αξιοπιστίας`, `Reliability Score`)}</div>
               <span className="text-[16.2px] font-bold text--400">{currentUser.reliabilityScore}%</span>
             </div>
             <p className="text-[13.8px] text-white mt-1.5 font-medium leading-relaxed">{t(`Βασίζεται στις παρουσίες, αξιολογήσεις και συμπεριφορά σας.`, `Based on your attendance, reviews, and behavior.`)}</p>
           </div>
         </Card>
      ...
      <Card className="p-5">
        <h3 className="text-[12.5px] font-bold text-white tracking-wide mb-4 flex items-center gap-1.5">
          <TrendingUp className="w-3.5 h-3.5 text-fuchsia-400" />{t(`Ιστορικό Εμπιστοσύνης`, `Trust History`)}
        </h3>
        <div className="space-y-0">
          {[
            ...
            <div key={i} className="flex gap-3">
              ...
              <div className="pb-4">
                <p className="text-[13.8px] font-medium text-white">{item.event}</p>
                <span className="text-[11.2px] text-white font-medium">{item.date}</span>
              </div>
            </div>
          ))}
        </div>
