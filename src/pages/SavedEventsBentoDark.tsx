...
export default function SavedEventsBentoDark() {
  const { t } = useLanguage();
  ...
  return (
    <div className="max-w-full mx-auto space-y-6 animate-in slide-in-from-bottom-4 duration-500 fade-in pb-20 md:pb-0">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <h1 className="text-xl md:text-2xl font-bold text-white">{t('Αποθηκευμένα', 'Saved Events')}</h1>
          <p className="text-white font-medium text-xs md:text-sm mt-1">{t('Εμπειρίες που έχετε προσθέσει στους σελιδοδείκτες σας.', 'Experiences you\'ve bookmarked for later.')}</p>
        </div>
      </div>
      ...
      {savedEvents.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {savedEvents.map(event => (
            <Link key={event.id} to={`/events/${event.id}`} className="bg-gray-800 border-gray-700 text-white rounded-xl border border-gray-700 overflow-hidden shadow-sm hover:shadow-md transition-all group flex flex-col">
              <div className="h-32 bg-gray-200 relative overflow-hidden shrink-0">
                ...
                <button className="absolute top-3 right-3 w-8 h-8 rounded-full bg-gray-800 border-gray-700 text-white flex items-center justify-center text-white shadow-sm z-10" aria-label={t('Αποθηκευμένο', 'Saved')}>
                  <Bookmark className="w-4 h-4 fill-current text-emerald-400" />
                </button>
              </div>
              <div className="p-4 flex flex-col flex-1">
                ...
                <h3 className="font-bold text-sm text-white group-hover:text-emerald-400 transition-colors line-clamp-2 mb-2">{event.title}</h3>
                ...
                <div className="mt-4 pt-3 border-t border-gray-800 flex items-center gap-1.5 text-[10px] font-bold text-emerald-400 tracking-wider">
        ...
        <div className="text-center py-16 bg-gray-800/70 rounded-2xl border border-dashed border-gray-700">
          ...
          <Link to="/" className="inline-flex items-center justify-center px-4 py-2 mt-4 bg-emerald-600 text-white text-xs font-bold rounded-full shadow-sm hover:bg-emerald-700 transition-colors">
