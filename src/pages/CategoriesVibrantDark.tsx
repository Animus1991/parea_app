...
export default function CategoriesVibrantDark() {
  ...
  const ALL_CATEGORIES = [
    ...
    { id: '3', name: t('Σινεμα', 'Cinema'), icon: Camera, count: 15, color: 'text-fuchsia-400', bg: 'bg-fuchsia-900/30', border: 'border-fuchsia-800' },
    ...
    { id: '9', name: t('Λεσχες βιβλιου', 'Book clubs'), icon: BookOpen, count: 6, color: 'text--400', bg: 'bg-fuchsia-950', border: 'border-fuchsia-900' },
    { id: '10', name: t('Ανταλλαγη γλωσσων', 'Language exchange'), icon: Languages, count: 14, color: 'text-fuchsia-400', bg: 'bg-fuchsia-900/30', border: 'border-fuchsia-800' },
    ...
    { id: '16', name: t('Περιπατοι στη φυση', 'Nature walks'), icon: TreePine || Leaf, count: 25, color: 'text--400', bg: 'bg-fuchsia-950', border: 'border-fuchsia-800' },
    ...
    { id: '21', name: t('Ιδιωτικες εκδηλωσεις', 'Private events'), icon: Lock, count: 8, color: 'text-fuchsia-400', bg: 'bg-fuchsia-900/30', border: 'border-fuchsia-800' }
  ];
    ...
    <div className="max-w-full mx-auto space-y-6 animate-in slide-in-from-bottom-4 duration-500 fade-in pb-20 md:pb-0">
      ...
      <div className="relative mb-6 max-w-md">
        ...
        <input 
          ...
          className="w-full h-11 pl-10 pr-4 rounded-full border border-gray-700 bg-gray-800 shadow-sm focus:outline-none focus:ring-2 focus:ring-fuchsia-600 focus:border-transparent text-sm font-medium transition-shadow hover:shadow-md"
        />
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 xl:grid-cols-6 gap-3">
        {visibleCategories.map(cat => (
          <div 
            key={cat.id} 
            className={`bg-gray-800 rounded-xl p-3 border shadow-sm hover:shadow-md transition-all cursor-pointer group flex flex-col items-center text-center ${activeCategory === cat.name ? 'border-fuchsia-600 ring-1 ring-fuchsia-600' : 'border-gray-800 hover:border-gray-700'}`}
          ...
          >
            ...
            <h3 className="font-bold text-white text-xs group-hover:text-fuchsia-400 transition-colors line-clamp-1 w-full">{cat.name}</h3>
            ...
            <div className="flex gap-1 shrink-0 bg-gray-700/80 p-0.5 rounded-lg border border-gray-700/50">
              {['All', 'Free', 'Paid', 'Group Discount'].map((price) => (
                <button
                  ...
                  className={`text-[10px] sm:text-xs font-semibold px-2.5 sm:px-3 py-1.5 rounded-md transition-all whitespace-nowrap ${priceFilter === price ? 'bg-gray-800 text-fuchsia-400 shadow-sm border border-gray-700/50 ring-1 ring-black/5' : 'text-white hover:text-white hover:bg-gray-200/50'}`}
                >
            ...
            <div className="flex gap-1 shrink-0 bg-gray-700/80 p-0.5 rounded-lg border border-gray-700/50">
              {['Any', 'Today', 'This Week', 'This Month'].map((date) => (
                <button
                  ...
                  className={`text-[10px] sm:text-xs font-semibold px-2.5 sm:px-3 py-1.5 rounded-md transition-all whitespace-nowrap ${dateFilter === date ? 'bg-gray-800 text-fuchsia-400 shadow-sm border border-gray-700/50 ring-1 ring-black/5' : 'text-white hover:text-white hover:bg-gray-200/50'}`}
                >
      ...
      <div className="mt-8 bg-fuchsia-900/30 rounded-2xl p-6 border border-fuchsia-800 flex flex-col md:flex-row items-center justify-between gap-4">
        <div>
        ...
        <button onClick={() => navigate('/create-event')} className="bg-fuchsia-600 text-white px-5 py-2 rounded-full text-xs font-bold shadow-sm hover:bg-fuchsia-700 transition-colors whitespace-nowrap shrink-0">
