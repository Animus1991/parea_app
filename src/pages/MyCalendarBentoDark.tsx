...
export default function MyCalendarBentoDark() {
  ...
  return (
    <div className="max-w-full mx-auto space-y-6 animate-in slide-in-from-bottom-4 duration-500 fade-in pb-20 md:pb-0">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        ...
        <div className="flex bg-gray-800 border border-gray-700 p-1 rounded-lg">
          <button 
            className={`px-4 py-1.5 rounded-md text-xs font-bold transition-colors ${view === 'week' ? 'bg-gray-800 border-gray-700 text-white shadow-sm text-white' : 'text-white'}`}
            onClick={() => setView('week')}
          >
            {t('Εβδομάδα', 'Week')}
          </button>
          <button 
            className={`px-4 py-1.5 rounded-md text-xs font-bold transition-colors ${view === 'month' ? 'bg-gray-800 border-gray-700 text-white shadow-sm text-white' : 'text-white'}`}
      ...
      <div className="bg-gray-800 border-gray-700 text-white rounded-2xl border border-gray-700 overflow-hidden shadow-sm">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-800">
          <h2 className="font-bold text-white text-lg">
            {format(currentDate, 'MMMM yyyy')}
          </h2>
          <div className="flex items-center gap-2">
            <button onClick={prevPeriod} className="p-1.5 hover:bg-gray-800 border border-gray-700 rounded-full transition-colors" aria-label={t('Προηγούμενη περίοδος', 'Previous period')}>
              <ChevronLeft className="w-5 h-5 text-white" />
            </button>
            <button onClick={() => setCurrentDate(new Date())} className="px-3 py-1 text-xs font-bold bg-gray-800 border border-gray-700 text-white rounded-md hover:bg-gray-200">
              {t('Σήμερα', 'Today')}
            </button>
            <button onClick={nextPeriod} className="p-1.5 hover:bg-gray-800 border border-gray-700 rounded-full transition-colors" aria-label={t('Επόμενη περίοδος', 'Next period')}>
                ...
                {monthDays.map(day => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {dayEvents.length > 0 && (
                        <div className={`w-1.5 h-1.5 rounded-full mt-0.5 ${isToday(day) ? 'bg-gray-800 border-gray-700 text-white' : 'bg-emerald-600'}`} />
                      )}
                    </div>
                  );
                })}
