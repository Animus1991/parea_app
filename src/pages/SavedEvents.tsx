    ...
    <div className="max-w-3xl mx-auto space-y-6 animate-in slide-in-from-bottom-4 duration-500 fade-in pb-20 md:pb-0">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 font-medium text-xs md:text-sm mt-1">Experiences you've bookmarked for later.</p>
        </div>
      </div>
      ...
      {savedEvents.length > 0 ? (
              ...
              <div className="p-4 flex flex-col flex-1">
                <div className="mb-2">
                  <Badge variant={event.safetyLevel === 'high_trust' ? 'warning' : 'outline'}>
                    {event.safetyLevel === 'high_trust' ? 'High Trust Req.' : event.safetyLevel === 'medium' ? 'Medium Trust' : 'Low Risk • Public'}
                  </Badge>
                </div>
                <h3 className="font-bold text-sm text-[#111827] group-hover:text-indigo-600 transition-colors line-clamp-2 mb-2">{event.title}</h3>
                ...
                <div className="mt-4 pt-3 border-t border-gray-100 flex items-center gap-1.5 text-[10px] uppercase font-bold text-indigo-600 tracking-wider">
        ...
        <div className="text-center py-16 bg-gray-50 rounded-2xl border border-dashed border-gray-200">
          ...
          <h3 className="text-base font-bold text-[#111827]">No saved events yet</h3>
          <p className="text-sm text-gray-500 mt-1 max-w-sm mx-auto">When you see an event you like, bookmark it to find it easily later.</p>
          <Link to="/" className="inline-flex items-center justify-center px-4 py-2 mt-4 bg-indigo-600 text-white text-xs font-bold rounded-full shadow-sm hover:bg-indigo-700 transition-colors">
            Discover Events
          </Link>
        </div>
      )}
