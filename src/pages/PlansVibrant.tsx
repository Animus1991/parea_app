...
export default function PlansVibrant() {
  const { t } = useLanguage();
    ...
    <div className="mx-auto max-w-full space-y-6 md:space-y-8 pb-20 md:pb-0">
      ...
      <Card className="rounded-xl p-4 border border-amber-200 bg-amber-50 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h3 className="font-bold text-[#111827] text-[15.921724499999998px] mb-1">{t(`Επαληθεύστε την ταυτότητά σας`, `Verify your identity`)}</h3>
      ...
      <div className="flex gap-4 border-b border-gray-200 overflow-x-auto noscrollbar">
        <button 
          ...
          className={`pb-3 text-[11.506875px] font-bold tracking-wide transition-colors whitespace-nowrap ${
            activeTab === 'upcoming' 
              ? 'border-b-2 border-fuchsia-600 text-fuchsia-900' 
      ...
      <div className="space-y-4">
        {upcomingEvents.map(event => (
          <Card key={event.id} className="rounded-xl p-4 sm:p-5 border border-fuchsia-100 flex flex-col sm:flex-row gap-4">
            <div className="w-full sm:w-32 h-32 sm:h-auto shrink-0 bg-gray-100 rounded-lg overflow-hidden relative">
              ...
              <div className="absolute top-2 left-2 bg-gradient-to-r from-fuchsia-600 to-violet-600 text-white px-2 py-0.5 rounded text-[10.6875px] font-bold tracking-wide">
            ...
            <div className="flex-1 flex flex-col">
              <div className="flex justify-between items-start gap-2 mb-2">
                ...
                <div className="text-right shrink-0">
                  ...
                  {(() => {
                    ...
                    if (days <= 0 && hours > 0) return <span className="text-[10.639999999999999px] font-bold text-orange-600 bg-orange-50 px-1.5 py-0.5 rounded mt-0.5 inline-block">{t(`σε`, `in`)} {hours}h</span>;
                    if (days > 0 && days <= 7) return <span className="text-[10.639999999999999px] font-bold text-fuchsia-700 bg-fuchsia-50 px-1.5 py-0.5 rounded mt-0.5 inline-block">{t(`σε`, `in`)} {days} {t(`μέρες`, `days`)}</span>;
                    return null;
                  })()}
                </div>
              </div>
              ...
              <div className="mt-auto pt-4 border-t border-gray-100 flex gap-2">
                <button onClick={() => navigate(`/chat/${event.id}`)} className="flex-1 bg-fuchsia-50 hover:bg-fuchsia-100 text-fuchsia-700 py-2 rounded-lg text-[12.825px] font-bold transition-colors flex items-center justify-center gap-1.5">
        ...
        {upcomingEvents.length === 0 && (
          <div className="text-center py-12 bg-gray-50 rounded-xl border border-dashed border-gray-200">
            ...
            <button className="bg-gradient-to-br from-violet-600 via-fuchsia-600 to-orange-500 text-white px-5 py-2 rounded-full text-[13.5375px] font-bold" onClick={() => navigate('/')}>{t(`Εξερεύνηση`, `Explore Experiences`)}</button>
          </div>
        )}
      </div>
      )}
        ...
        {pastEvents.map((event, index) => (
          <Card key={event.id} className="rounded-xl p-4 sm:p-5 border border-gray-200 flex flex-col sm:flex-row gap-4">
            <div className="flex-1 flex flex-col">
              ...
              <h3 className="font-bold text-[19px] text-[#111827] mb-3">{event.title}</h3>
              
              {index === 0 ? (
                <div className="bg-fuchsia-50 border border-fuchsia-100 p-3 rounded-lg flex items-center justify-between mt-auto">
                  <div className="text-[12.825px] font-bold text-fuchsia-900">{t(`Απαιτείται αξιολόγηση`, `Feedback required`)}</div>
                  <button onClick={() => navigate(`/history/feedback/${event.id}`)} className="text-[10.6875px] font-bold bg-gradient-to-r from-fuchsia-600 to-orange-500 text-white px-3 py-1.5 rounded hover:opacity-90 tracking-wide">
                    {t(`Αξιολόγηση`, `Rate Experience`)}
                  </button>
                </div>
              ) : (
                <div className="flex items-center gap-2 mt-auto text-[12.825px] font-bold text-emerald-600">
