...
export default function MyCalendarNeon() {
    ...
    <div className="max-w-full mx-auto space-y-6 animate-in slide-in-from-bottom-4 duration-500 fade-in pb-20 md:pb-0">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-black font-medium text-xs md:text-sm mt-1">{t('Διαχειριστείτε το πρόγραμμά σας και τις προσεχείς εμπειρίες.', 'Manage your schedule and upcoming experiences.')}</p>
        </div>
        
        <div className="flex bg-gray-100 p-1 rounded-lg">
          <button 
            className={`px-4 py-1.5 rounded-md text-xs font-bold transition-colors ${view === 'week' ? 'bg-white shadow-sm text-[#111827]' : 'text-black'}`}
            onClick={() => setView('week')}
          >
            {t('Εβδομάδα', 'Week')}
          </button>
          <button 
            className={`px-4 py-1.5 rounded-md text-xs font-bold transition-colors ${view === 'month' ? 'bg-white shadow-sm text-[#111827]' : 'text-black'}`}
            ...
            >
              <div className="grid grid-cols-7 gap-2 mb-4">
                {days.map((day, i) => (
                  <div key={i} className="text-center">
                    ...
                    <div className={`mt-1 w-8 h-8 mx-auto flex items-center justify-center rounded-full text-sm font-bold ${isToday(day) ? 'bg-emerald-600 text-white' : 'text-[#111827]'}`}>
                      {format(day, 'd')}
                    </div>
                  </div>
                ))}
              ...
              <div className="space-y-3 mt-6">
                {upcomingEvents.map((event, idx) => (
                  <div key={idx} className="flex gap-4 p-4 rounded-xl border border-gray-100 hover:border-emerald-100 hover:bg-emerald-50/30 transition-colors cursor-pointer" onClick={() => navigate(`/events/${event.id}`)}>
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex flex-wrap items-center gap-3 mt-2 text-xs text-black font-medium">
            ...
            >
              ...
              <div className="grid grid-cols-7 gap-1 mb-1">
                {[t('Δευ', 'Mon'), t('Τρι', 'Tue'), t('Τετ', 'Wed'), t('Πεμ', 'Thu'), t('Παρ', 'Fri'), t('Σαβ', 'Sat'), t('Κυρ', 'Sun')].map(d => (
                  <div key={d} className="text-center text-[10px] font-bold text-black tracking-wide py-2">{d}</div>
                ))}
              </div>
                ...
                {monthDays.map(day => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {dayEvents.length > 0 && (
                        <div className={`w-1.5 h-1.5 rounded-full mt-0.5 ${isToday(day) ? 'bg-white' : 'bg-emerald-600'}`} />
                      )}
                    </div>
                  );
                })}
              ...
              {upcomingEvents.length > 0 && (
                <div className="mt-4 space-y-2 border-t border-gray-100 pt-4">
                  <h4 className="text-[10px] font-bold text-black tracking-wide mb-2">{t('Προσεχώς αυτόν τον μήνα', 'Upcoming This Month')}</h4>
                  {upcomingEvents.map((event, idx) => (
                    <div
                      key={idx}
                      className="flex gap-3 items-center text-sm cursor-pointer hover:bg-emerald-50 p-2 rounded-lg transition-colors"
