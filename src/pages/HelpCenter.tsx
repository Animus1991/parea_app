  ...
  return (
    <div className="max-w-full mx-auto space-y-6 animate-in slide-in-from-bottom-4 duration-500 fade-in pb-20 md:pb-0">
          ...
          {faqs.map(faq => (
            <Card key={faq.id} className="overflow-hidden">
              ...
              >
                ...
                <ChevronDown className={`w-4 h-4 text-gray-400 shrink-0 transition-transform ${expandedFAQ === faq.id ? 'rotate-180' : ''}`} />
              </button>
              {expandedFAQ === faq.id && (
                <div className="px-4 pb-4 -mt-1">
                  <p className="text-[13.5px] text-gray-600 font-medium leading-relaxed">{faq.a}</p>
                </div>
              )}
            </Card>
          ))}
      ...
      <Card className="p-6 text-center">
        ...
        <p className="text-[13.5px] text-gray-500 mb-2">{t(`Επικοινωνήστε μαζί μας`, `Contact our support team`)}</p>
