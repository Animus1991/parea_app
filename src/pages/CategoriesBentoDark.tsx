...
export default function CategoriesBentoDark() {
      ...
      <div className="mt-8 border-t border-gray-700 pt-8" id="events-list">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
          ...
          <div className="flex gap-2 overflow-x-auto pb-2 noscrollbar items-center w-full md:w-auto">
            ...
            {/* Price Pills */}
            <div className="flex gap-1 shrink-0 bg-gray-800 border border-gray-700/80 p-0.5 rounded-lg border border-gray-700/50">
              {['All', 'Free', 'Paid', 'Group Discount'].map((price) => (
                <button
                  key={price}
                  onClick={() => setPriceFilter(price as any)}
                  className={`text-[10px] sm:text-xs font-semibold px-2.5 sm:px-3 py-1.5 rounded-md transition-all whitespace-nowrap ${priceFilter === price ? 'bg-gray-800 border-gray-700 text-white text-emerald-400 shadow-sm border border-gray-700/50 ring-1 ring-black/5' : 'text-white hover:text-white hover:bg-gray-200/50'}`}
                >
            ...
            <div className="flex gap-1 shrink-0 bg-gray-800 border border-gray-700/80 p-0.5 rounded-lg border border-gray-700/50">
              {['Any', 'Today', 'This Week', 'This Month'].map((date) => (
                <button
                  ...
                  className={`text-[10px] sm:text-xs font-semibold px-2.5 sm:px-3 py-1.5 rounded-md transition-all whitespace-nowrap ${dateFilter === date ? 'bg-gray-800 border-gray-700 text-white text-emerald-400 shadow-sm border border-gray-700/50 ring-1 ring-black/5' : 'text-white hover:text-white hover:bg-gray-200/50'}`}
                >
