...
export default function PlansVibrantDark() {
  const { t } = useLanguage();
    ...
    <div className="mx-auto max-w-full space-y-6 md:space-y-8 pb-20 md:pb-0">
      ...
      <div className="flex gap-4 border-b border-gray-700 overflow-x-auto noscrollbar">
        <button 
          ...
          className={`pb-3 text-[11.506875px] font-bold tracking-wide transition-colors whitespace-nowrap ${
            activeTab === 'upcoming' 
              ? 'border-b-2 border-fuchsia-600 text-fuchsia-400' 
      ...
      <div className="space-y-4">
        {upcomingEvents.map(event => (
          <Card key={event.id} className="rounded-xl p-4 sm:p-5 border border-fuchsia-800 flex flex-col sm:flex-row gap-4">
            <div className="w-full sm:w-32 h-32 sm:h-auto shrink-0 bg-gray-700 rounded-lg overflow-hidden relative">
              ...
              <div className="absolute top-2 left-2 bg-fuchsia-600 text-white px-2 py-0.5 rounded text-[10.6875px] font-bold tracking-wide">
            ...
            <div className="flex-1 flex flex-col">
              <div className="flex justify-between items-start gap-2 mb-2">
                ...
                <div className="text-right shrink-0">
                  ...
                  {(() => {
                    ...
                    if (days > 0 && days <= 7) return <span className="text-[10.639999999999999px] font-bold text-fuchsia-400 bg-fuchsia-900/30 px-1.5 py-0.5 rounded mt-0.5 inline-block">{t(`σε`, `in`)} {days} {t(`μέρες`, `days`)}</span>;
                    return null;
                  })()}
                </div>
              </div>
              ...
              <div className="mt-auto pt-4 border-t border-gray-800 flex gap-2">
                <button onClick={() => navigate(`/chat/${event.id}`)} className="flex-1 bg-fuchsia-900/30 hover:bg-fuchsia-900/50 text-fuchsia-400 py-2 rounded-lg text-[12.825px] font-bold transition-colors flex items-center justify-center gap-1.5">
        ...
        {pastEvents.map((event, index) => (
          <Card key={event.id} className="rounded-xl p-4 sm:p-5 border border-gray-700 flex flex-col sm:flex-row gap-4">
            <div className="flex-1 flex flex-col">
              ...
              {index === 0 ? (
                <div className="bg-fuchsia-900/30 border border-fuchsia-800 p-3 rounded-lg flex items-center justify-between mt-auto">
                  <div className="text-[12.825px] font-bold text-fuchsia-400">{t(`Απαιτείται αξιολόγηση`, `Feedback required`)}</div>
                  <button onClick={() => navigate(`/history/feedback/${event.id}`)} className="text-[10.6875px] font-bold bg-fuchsia-600 text-white px-3 py-1.5 rounded hover:bg-fuchsia-700 tracking-wide">
