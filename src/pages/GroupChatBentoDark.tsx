...
export default function GroupChatBentoDark() {
  const { t } = useLanguage();
        ...
        <div className="flex items-center gap-2">
          ...
          <div
            className={`max-w-[85%] sm:max-w-[75%] md:max-w-[65%] rounded-2xl px-4 py-2.5 text-base leading-relaxed relative ${
              isMe
                ? "bg-gray-800 shadow-[0_8px_30px_rgb(0,0,0,0.5)] border border-gray-700 text-white rounded-tr-sm shadow-sm"
                : "bg-gray-800 border-gray-700 text-white border border-gray-700 text-white rounded-tl-sm shadow-sm"
            }`}
          >
            {msg.text}
          </div>
  ...
  return (
    <div className="w-full h-full px-[5px] md:px-0 mx-auto max-w-[1200px]">
      <div className="w-full h-[calc(100dvh-5rem)] md:h-[calc(100dvh-6rem)] flex bg-gray-800 border-gray-700 text-white border border-gray-700 rounded-xl shadow-sm overflow-hidden relative">
        <div
          className={`flex flex-col flex-1 h-full min-w-0 ${showInfo ? "hidden md:flex border-r border-gray-700" : "flex"}`}
        >
          {/* Header */}
          <div className="flex items-center justify-between px-3 md:px-5 py-3 border-b border-gray-700 bg-gray-800 border-gray-700 text-white shrink-0 relative z-20">
            {showSearchMobile ? (
              ...
              <>
                <div className="flex items-center gap-3 overflow-hidden">
                  <button
                    onClick={() => navigate(-1)}
                    className="text-white hover:text-white transition-colors p-1 shrink-0"
                  >
                    <ArrowLeft className="h-5 w-5" />
                  </button>
                ...
                <div className="flex items-center justify-end gap-1 sm:gap-2 shrink-0">
                  ...
                  <div className="hidden md:flex items-center gap-3 mr-2">
                    <select
                      ...
                      className="text-xs font-bold border border-gray-700 rounded-lg px-2.5 py-1.5 focus:outline-none focus:ring-1 focus:ring-emerald-9500 bg-gray-800/70 text-white hover:bg-gray-800 border border-gray-700 cursor-pointer transition-colors max-w-[120px] truncate"
                    >
                      <option value="all">
                  ...
                  <button
                    ...
                    className="text-white hover:text-white hover:bg-gray-800 border border-gray-700 rounded-full transition-colors p-2 md:hidden"
                  >
                  ...
                  <button
                    ...
                    className={`transition-colors p-2 rounded-full ${showInfo ? "text-emerald-400 bg-emerald-900/30" : "text-white hover:text-white hover:bg-gray-800 border border-gray-700"}`}
                  >
            ...
            {isSharingLocation && (
              <div className="absolute top-4 left-1/2 -translate-x-1/2 z-20">
                <button
                  ...
                  className="bg-gray-800 border-gray-700 text-white/90 backdrop-blur-sm border border-emerald-200 text--400 shadow-sm px-4 py-2 rounded-full text-xs font-bold flex items-center gap-2"
                >
          ...
          <form
            onSubmit={handleSend}
            className="p-3 sm:p-4 border-t border-gray-700 bg-gray-800 border-gray-700 text-white shrink-0 relative z-20"
          >
            <div className="flex items-center gap-2 md:gap-3 max-w-4xl mx-auto w-full relative">
              <button
                ...
                className={`p-2.5 rounded-full transition-all flex items-center justify-center shrink-0 border hidden sm:flex ${
                  isSharingLocation
                    ...
                    : "bg-gray-900 text-white border-gray-700 hover:bg-gray-800 border border-gray-700 hover:text-white"
                }`}
              ...
              <div className="flex-1 relative">
                <input
                  ...
                  className="w-full bg-gray-800 border border-gray-700 border border-transparent rounded-full py-3 md:py-3.5 pl-5 pr-14 text-sm focus:bg-gray-800 border-gray-700 text-white focus:ring-2 focus:ring-[#111827] focus:border-[#111827] outline-none transition-all shadow-sm"
                />
                <button
                  type="submit"
                  disabled={!newMessage.trim()}
                  className="absolute right-1.5 top-1/2 -translate-y-1/2 p-2 bg-gray-800 shadow-[0_8px_30px_rgb(0,0,0,0.5)] border border-gray-700 text-white rounded-full hover:bg-gray-800 disabled:opacity-40 disabled:hover:bg-gray-800 text-white border border-gray-700 shadow-[0_8px_30px_rgb(0,0,0,0.5)] transition-all shadow-sm"
                >
        ...
        {showInfo && (
          <div className="w-full md:w-[320px] lg:w-[380px] bg-gray-800 border-gray-700 text-white shrink-0 flex flex-col absolute md:relative z-30 h-full border-l border-gray-700 animate-in slide-in-from-right-8 duration-200">
            <div className="flex items-center justify-between p-4 border-b border-gray-700 bg-gray-800 border-gray-700 text-white sticky top-0 z-10 shadow-sm">
              <h3 className="font-bold text-white text-sm tracking-wide">
                {t("Λεπτομέρειες Ομάδας", "Group Details")}
              </h3>
              <button
                onClick={() => setShowInfo(false)}
                className="text-white hover:text-white p-1 rounded-full hover:bg-gray-800 border border-gray-700 transition-colors"
              >
                <X className="h-5 w-5" />
              </button>
            </div>

            <div className="flex-1 overflow-y-auto custom-scrollbar p-5 space-y-6 bg-gray-900/50">
              {/* Event Summary */}
              <div className="bg-gray-800 border-gray-700 text-white rounded-xl shadow-sm border border-gray-700 overflow-hidden">
                ...
                <div className="p-4 space-y-3">
                  {event.tags && event.tags.length > 0 && (
                    <div className="flex gap-1.5 flex-wrap">
                      {event.tags.map((tag: string) => (
                        <span
                          key={tag}
                          className="bg-emerald-900/30 text-emerald-400 px-2.5 py-1 rounded-md text-xs font-bold tracking-wide border border-emerald-800"
                        >
                          {tag}
                        </span>
                      ))}
                  ...
                  <div className="space-y-2.5 text-sm font-medium text-white">
                    <div className="flex items-start gap-3">
                      <div className="p-1.5 bg-gray-800 border border-gray-700 rounded-md text-white shrink-0">
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <span className="pt-0.5 text-white font-semibold">
              ...
              {event.groupDiscount && (
                <div
                  className={`p-4 rounded-xl border shadow-sm ${group.discountUnlocked ? "bg-emerald-50 border-emerald-200" : "bg-gray-800 border-gray-700 text-white border-gray-700"}`}
                >
                  <div className="flex items-center gap-2.5 mb-2">
                    <div
                      className={`p-1.5 rounded-full ${group.discountUnlocked ? "bg-emerald-100 text--400" : "bg-gray-800 border border-gray-700 text-white"}`}
                    >
                      <Tag className="h-4 w-4" />
                    </div>
                    <span
                      className={`font-bold text-sm ${group.discountUnlocked ? "text--400" : "text-white"}`}
                    >
                      {t("Ομαδική Έκπτωση", "Group Discount")}
                    </span>
                  </div>
                  <p
                    className={`text-xs font-medium leading-relaxed ${group.discountUnlocked ? "text--400" : "text-white"}`}
                  >
              ...
              <div className="bg-gray-800 border-gray-700 text-white rounded-xl shadow-sm border border-gray-700 p-4">
                  ...
                  <div className="flex items-center gap-2">
                    {isGroupAdmin && (
                      <button
                        ...
                        className="flex items-center gap-1 text-xs font-bold text-emerald-400 hover:text-emerald-400 bg-emerald-900/30 hover:bg-emerald-900/50 px-2 py-1 rounded border border-emerald-800 transition-colors"
                      >
                        <UserPlus className="w-3 h-3" />{" "}
                        {t("Προσθήκη Μελών", "Add Members")}
                      </button>
                    )}
                    <span className="text-xs font-bold text-white bg-gray-800 border border-gray-700 px-2 py-0.5 rounded-full">
                      {group.members.length}/{group.targetSize}
                    </span>
                  </div>
                </div>
                <div className="relative mb-4">
                  <Search className="h-3.5 w-3.5 text-white absolute left-3 top-1/2 -translate-y-1/2" />
                  <input
                    type="text"
              ...
              {/* Chat Settings */}
              <div className="bg-gray-800 border-gray-700 text-white rounded-xl shadow-sm border border-gray-700 p-4 space-y-5">
                <div>
                  ...
                  <div className="flex items-center justify-between gap-4">
                    ...
                    <button
                      ...
                      className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer items-center rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-[#111827] focus:ring-offset-2 ${isEphemeral ? "bg-gray-800 text-white border border-gray-700 shadow-[0_8px_30px_rgb(0,0,0,0.5)]" : "bg-gray-200"}`}
                    >
                      <span className="sr-only">{t('Εναλλαγή εφήμερης λειτουργίας', 'Toggle ephemeral mode')}</span>
                      <span
                        aria-hidden="true"
                        className={`pointer-events-none inline-block h-5 w-5 transform rounded-full bg-gray-800 border-gray-700 text-white shadow ring-0 transition duration-200 ease-in-out ${isEphemeral ? "translate-x-5" : "translate-x-0"}`}
                      />
                    </button>
                  </div>
                </div>
                ...
                <div className="pt-4 border-t border-gray-800">
                  ...
                  <div className="space-y-2">
                    ...
                    >
                      <div>
                        <p
                          className={`text-sm font-bold ${isSharingLocation ? "text-emerald-400" : "text-white"}`}
                        >
                          {isSharingLocation
                        ...
                        <p
                          className={`text-xs mt-0.5 ${isSharingLocation ? "text-emerald-400/80" : "text-white"}`}
                        >
                          {isSharingLocation
                    ...
                    >
                      ...
                      <ShieldCheck className="w-4 h-4 text--400" />
                    </button>
                    ...
                    >
                      ...
                      <Link2 className="h-4 w-4 text--400" />
                    </button>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-2 pt-2">
                ...
                <button
                  ...
                  className="w-full py-2.5 px-4 text-xs font-bold tracking-wide text-white bg-gray-800 border-gray-700 text-white hover:bg-gray-700/60 border border-gray-700 rounded-lg transition-colors"
                >
        ...
        {showStatusModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-gray-800 border-gray-700 text-white rounded-2xl shadow-xl max-w-sm w-full p-6 text-center border border-gray-800 animate-in zoom-in-95 duration-200">
              <div className="w-12 h-12 bg-gray-800 border border-gray-700 rounded-full flex items-center justify-center mx-auto mb-4 text-white">
                <Clock className="w-6 h-6" />
              </div>
              <h3 className="text-lg font-bold text-white mb-2">
                {t("Κοινοποίηση Κατάστασης", "Share Arrival Status")}
              </h3>
              <p className="text-xs font-medium leading-relaxed text-white mb-6">
                {t(
        ...
        {showLocationConfigModal && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-gray-800 border-gray-700 text-white rounded-2xl shadow-xl max-w-md w-full max-h-[90vh] overflow-y-auto custom-scrollbar border border-gray-800 animate-in zoom-in-95 duration-200">
              <div className="p-5 border-b border-gray-800 flex items-center justify-between sticky top-0 bg-gray-800 border-gray-700 text-white/95 backdrop-blur-sm z-10">
                ...
                <button
                  ...
                  className="text-white hover:text-white p-1.5 rounded-full hover:bg-gray-800 border border-gray-700"
                >
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              <div className="p-5 space-y-6">
                {isSharingLocation && (
                  <div className="bg-emerald-900/30 border border-emerald-800 rounded-xl p-4 flex flex-col gap-3">
                    ...
                    <button
                      ...
                      className="w-full bg-gray-800 border-gray-700 text-white text--400 border border-red-200 hover:bg-red-50 py-2 rounded-lg text-xs font-bold tracking-wide"
                    >
                ...
                <div className="space-y-3">
                  ...
                  <select
                    ...
                    className="w-full border border-gray-700 rounded-xl p-3 text-sm font-medium focus:ring-2 focus:ring-[#111827] bg-gray-800 border-gray-700 text-white outline-none"
                  >
        ...
        {showReportModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-gray-800 border-gray-700 text-white rounded-2xl shadow-xl max-w-md w-full p-6 border border-gray-800 animate-in zoom-in-95 duration-200">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  ...
                  <p className="text-xs font-semibold text-white tracking-tight capitalize mt-0.5">
              ...
              <textarea
                className="w-full border border-gray-700 rounded-xl p-3 text-sm resize-none mb-5 focus:ring-2 focus:ring-[#111827] outline-none shadow-sm font-medium transition-all"
                rows={4}
                placeholder={t(
              ...
              <div className="flex justify-end gap-3">
                ...
                <button
                  onClick={() => {
                    toast.success(t("Η αναφορά σας υποβλήθηκε με ασφάλεια.", "Your report has been submitted securely."));
        ...
        {showStatusModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-gray-800 border-gray-700 text-white rounded-2xl shadow-xl max-w-sm w-full p-6 text-center border border-gray-800 animate-in zoom-in-95 duration-200">
              <div className="w-12 h-12 bg-blue-50 rounded-full flex items-center justify-center mx-auto mb-4 text--400">
                <Clock className="w-6 h-6" />
              </div>
              <h3 className="text-lg font-bold text-white mb-2">
                {t("Κατάσταση ’φιξης", "Share Arrival Status")}
        ...
        {showAddMemberModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-gray-800 border-gray-700 text-white rounded-2xl shadow-xl max-w-md w-full max-h-[85vh] flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
              <div className="p-4 sm:p-5 border-b border-gray-800 flex items-center justify-between bg-gray-800 border-gray-700 text-white relative z-10">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 bg-emerald-900/30 rounded-full flex items-center justify-center text-emerald-400">
                ...
                <button
                  ...
                  className="text-white hover:text-white p-1.5 rounded-full hover:bg-gray-800 border border-gray-700 transition-colors"
              ...
              <div className="p-4 sm:p-5 bg-gray-900/50 border-b border-gray-800">
                <div className="relative">
                  <Search className="h-4 w-4 text-white absolute left-3 top-1/2 -translate-y-1/2" />
                  <input
                    type="text"
                    placeholder={t(
                    ...
                    className="w-full pl-9 pr-3 py-2.5 text-sm border border-gray-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-600 bg-gray-800 border-gray-700 text-white transition-all shadow-sm font-medium"
                  />
                </div>
              </div>
              ...
              <div className="flex-1 overflow-y-auto custom-scrollbar p-2">
                {potentialNewMembers.length > 0 ? (
                    ...
                    .map((member) => (
                      ...
                      >
                        ...
                        <button
                          ...
                          className="text-sm font-semibold capitalize tracking-tight px-3 py-1.5 rounded-lg border flex items-center gap-1 transition-all bg-emerald-600 text-white border-emerald-600 hover:bg-emerald-700 shadow-sm"
                        >
                          <UserPlus className="w-3 h-3" />{" "}
                          {t("Προσθήκη", "Add")}
                        </button>
                  ...
                  <div className="text-center p-8 text-white flex flex-col items-center">
                    <div className="w-12 h-12 bg-gray-800 border border-gray-700 rounded-full flex items-center justify-center mb-3 text-white">
        ...
        {showLiveRadarModal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-gray-800 border-gray-700 text-white rounded-2xl shadow-xl w-full max-w-4xl max-h-[90vh] flex flex-col border border-gray-800 animate-in zoom-in-95 duration-200 overflow-hidden relative">
              <button
                onClick={() => setShowLiveRadarModal(false)}
                className="absolute top-4 right-4 z-[200] bg-gray-800 border-gray-700 text-white rounded-full p-2 shadow-sm border border-gray-700 text-white hover:text-white transition-colors"
              >
                <X className="w-5 h-5" />
              </button>
