...
export default function AchievementsClassic() {
  ...
  const uniqueCategories = new Set(events.filter(e => userGroups.some(g => g.eventId === e.id)).map(e => e.category)).size;

  const achievements: Achievement[] = [
    { id: 'a1', icon: Calendar, title: t(`Πρώτη Εμπειρία`, `First Experience`), description: t(`Συμμετείχατε στην πρώτη σας εκδήλωση`, `Attended your first event`), progress: Math.min(userGroups.length, 1), maxProgress: 1, unlocked: userGroups.length >= 1, xp: 50, color: 'text-cyan-600 bg-cyan-100' },
    ...
    { id: 'a3', icon: Star, title: t(`Αστέρι Αξιοπιστίας`, `Reliability Star`), description: t(`Διατηρήστε 95%+ αξιοπιστία`, `Maintain 95%+ reliability score`), progress: currentUser?.reliabilityScore ?? 0, maxProgress: 95, unlocked: (currentUser?.reliabilityScore ?? 0) >= 95, xp: 200, color: 'text-amber-600 bg-amber-100' },
    { id: 'a4', icon: Flame, title: t(`Εβδομαδιαίο Σερί`, `Weekly Streak`), description: t(`Συμμετοχή σε εκδηλώσεις 4 εβδομάδες σερί`, `Attend events for 4 weeks in a row`), progress: Math.min(feedbackCount, 4), maxProgress: 4, unlocked: feedbackCount >= 4, xp: 150, color: 'text-orange-600 bg-orange-100' },
    { id: 'a5', icon: Shield, title: t(`Αξιόπιστο Μέλος`, `Trusted Member`), description: t(`Επαληθεύστε ταυτότητα + τηλέφωνο + email`, `Verify ID + phone + email`), progress: currentUser?.badges?.length ?? 0, maxProgress: 3, unlocked: (currentUser?.badges?.length ?? 0) >= 3, xp: 75, color: 'text-green-600 bg-green-100' },
    ...
    { id: 'a7', icon: Trophy, title: t(`Πρωτοπόρος`, `Trailblazer`), description: t(`Δημιουργήστε μια ομάδα που γέμισε`, `Create a group that filled up`), progress: groups.some(g => g.hostId === currentUser?.id) ? 1 : 0, maxProgress: 1, unlocked: groups.some(g => g.hostId === currentUser?.id), xp: 100, color: 'text-rose-600 bg-rose-100' },
  ...
  const streakCount = Math.min(userGroups.length, 7);
  ...
  return (
    <div className="mx-auto max-w-full space-y-6 animate-in slide-in-from-bottom-4 duration-500 fade-in pb-20 md:pb-0">
      <div>
        <h1 className="text-[16px] md:text-[18px] font-bold text-[#111827]">{t(`Επιτεύγματα`, `Achievements`)}</h1>
      ...
      {/* Daily Challenge */}
      <Card className="p-4 bg-gradient-to-r from-purple-50 to-cyan-50 !border-purple-100/60 !rounded-2xl shadow-soft">
      ...
      <TabBar
        tabs={[
          { id: 'badges', label: t('Badges', 'Badges'), count: achievements.filter(a => a.unlocked).length },
          { id: 'streaks', label: t('Σερί', 'Streaks') },
          { id: 'leaderboard', label: t('Κατάταξη', 'Leaderboard') },
        ]}
        activeTab={activeTab}
        onChange={(id) => setActiveTab(id as 'badges' | 'streaks' | 'leaderboard')}
      />
              ...
              <Card key={a.id} className={`p-4 transition-all ${a.unlocked ? 'border-gray-200' : 'opacity-60'}`}>
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex flex-col items-end gap-1 shrink-0">
                    <span className="text-[11.2px] font-bold text-[#0E8B8D] bg-[#18D8DB]/[0.06] px-2 py-0.5 rounded-full">+{a.xp}XP</span>
                    {a.unlocked && (
        ...
        <div className="space-y-4">
          ...
          <Card className="p-5">
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div className="text-center">
                <p className="text-[23px] font-black text-[#111827]">{userGroups.length}</p>
