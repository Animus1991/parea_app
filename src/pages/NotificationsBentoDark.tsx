...
export default function NotificationsBentoDark() {
    const { t } = useLanguage();
  ...
  const notifications = [
    ...
    },
    {
      ...
      read: true,
      icon: Calendar,
      color: 'bg-gray-800 border border-gray-700 text-white'
    },
    {
      id: 'n4',
      type: 'system',
  ...
  return (
    <div className="mx-auto max-w-full space-y-6 pb-20 md:pb-0">
      <div className="flex justify-between items-end">
        ...
        <button className="text-[14.2457535px] font-bold text-emerald-400 hover:text-emerald-400">
      ...
      {todayNotifs.length > 0 && (
        <div className="space-y-3">
          <h3 className="text-[12.1125px] font-bold text-white tracking-wide">{t(`Σήμερα`, `Today`)}</h3>
      ...
      {earlierNotifs.length > 0 && (
        <div className="space-y-3">
          <h3 className="text-[11.2px] font-bold text-white tracking-wide">{t(`Προηγούμενες`, `Earlier`)}</h3>
          {earlierNotifs.map((notif) => {
            const Icon = notif.icon;
            return (
              <div key={notif.id} className="flex gap-4 p-4 rounded-xl border border-gray-800 bg-gray-800 border-gray-700 text-white">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${notif.color}`}>
                  <Icon className="w-5 h-5" />
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-[13.5px] leading-relaxed text-white">{notif.message}</p>
                  <span className="text-[11.25px] font-bold text-white tracking-wide mt-1 block">{notif.time}</span>
                </div>
              </div>
            );
