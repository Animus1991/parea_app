...
export default function OrganizerProfileBentoDark() {
  const { id } = useParams();
  const { t } = useLanguage();
  ...
  return (
    <div className="mx-auto max-w-full space-y-8 relative">
      {/* Profile Header */}
      <div className="bg-gray-800 border-gray-700 text-white rounded-xl border border-gray-700 p-8 shadow-sm text-center sm:text-left sm:flex sm:items-start gap-8">
        <div className="w-32 h-32 rounded-full overflow-hidden bg-gray-800 border border-gray-700 mx-auto sm:mx-0 shrink-0 border-4 border-white shadow-md">
          {organizer.photoUrl ? (
        ...
        <div className="mt-6 sm:mt-0 flex-1 space-y-4">
          <div>
            <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2 mb-2">
              {isVerifiedOrganizer && (
                <Badge variant="outline" icon={<ShieldCheck className="h-3 w-3 text-emerald-400" />}>
          ...
          <div className="flex flex-wrap items-center justify-center sm:justify-start gap-4 pt-2">
            <button 
              ...
              className="text-[10px] font-bold tracking-wide text-white hover:text-emerald-400 transition-colors flex items-center gap-1"
            >
              ...
              <Link key={event.id} to={`/events/${event.id}`}>
                <Card className="flex items-start gap-4 p-4 hover:border-cyan-300 transition-colors cursor-pointer group">
                  ...
                  <div className="flex-1">
                    <h3 className="font-bold text-white group-hover:text-emerald-400 transition-colors line-clamp-1">{event.title}</h3>
                  ...
                  <div className="hidden sm:flex items-center text-[10px] font-bold tracking-wide text-emerald-400 self-center">
                    {t('Προβολή', 'View')} &rarr;
                  </div>
                </Card>
              </Link>
      ...
      {showContactModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
          <div className="bg-gray-800 border-gray-700 text-white rounded-xl shadow-xl max-w-md w-full p-6">
            ...
            <div className="flex justify-end gap-3">
              <button 
                ...
                className="px-4 py-2 text-sm font-bold text-white hover:bg-gray-800 border border-gray-700 rounded-lg transition-colors"
              >
                {t('Ακύρωση', 'Cancel')}
              </button>
              <button 
