...
export default function NotFoundNeon() {
  const { t } = useLanguage();
  ...
  return (
    <div className="flex flex-col items-center justify-center py-16 px-6 text-center animate-in fade-in duration-500">
      <div className="w-20 h-20 rounded-full bg-gradient-to-br from-emerald-100 to-teal-100 flex items-center justify-center mb-6 relative">
        <span className="absolute inset-0 rounded-full bg-emerald-200 animate-ping opacity-20"></span>
        <Compass className="w-10 h-10 text-emerald-600 animate-spin" style={{ animationDuration: '8s' }} />
      </div>
      ...
      <div className="grid grid-cols-2 gap-2 mb-6 w-full max-w-xs">
        {suggestions.map(s => {
          const Icon = s.icon;
          return (
            <button key={s.path} onClick={() => navigate(s.path)} className="flex items-center gap-2 p-3 rounded-xl border border-gray-200 bg-white hover:border-emerald-300 hover:bg-emerald-50 transition-all text-left">
