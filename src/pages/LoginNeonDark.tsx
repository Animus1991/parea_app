...
export default function LoginNeonDark() {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen px-4 py-10 bg-gradient-to-br from-gray-950 via-emerald-950 to-gray-950">
      <LoginGuestHeroStrip variant="dark" accentClass="bg-emerald-900/40 text-emerald-400" />
      <Card className="w-full max-w-md p-8 space-y-6 animate-in fade-in slide-in-from-bottom-4 shadow-2xl border-gray-800 bg-gray-800/70">
        ...
        <div className="space-y-4 pt-4">
          ...
          <div className="text-center text-xs font-bold text-white tracking-wide mb-4">
          ...
          <div className="grid grid-cols-1 gap-3">
            {users.slice(0, 3).map(user => (
              <button 
                ...
                className="w-full flex items-center gap-3 px-4 py-3 rounded-xl border border-gray-700 bg-gray-800 hover:bg-emerald-900/30 hover:border-emerald-800 transition-colors shadow-sm text-left group"
              >
                <div className="h-10 w-10 rounded-full overflow-hidden bg-emerald-900/50 flex-shrink-0">
                  {user.photoUrl ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center text-emerald-400 font-bold">{user.name.substring(0, 2)}</div>
                  )}
                </div>
                <div>
                  <div className="text-sm font-bold text-white group-hover:text-emerald-400">{user.name}</div>
                  <div className="text-xs text-white">{user.isOrganizer ? t('Διοργανωτής', 'Organizer') : t('Χρήστης', 'User')} • {user.city}</div>
                </div>
              </button>
            ))}
          </div>
          ...
          <div className="relative py-4">
            ...
            <div className="relative flex justify-center">
              <span className="bg-gray-800 px-2 text-xs text-white tracking-wide">{t('Ή συνεχίστε με', 'Or continue with')}</span>
            </div>
          </div>

          <button 
            onClick={() => handleLogin(users[0].id)}
            className="w-full flex items-center justify-center gap-3 px-4 py-3 rounded-full border border-gray-700 bg-gray-800 text-sm font-bold text-white hover:bg-gray-700/60 transition-colors shadow-sm"
          >
