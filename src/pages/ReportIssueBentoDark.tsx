...
export default function ReportIssueBentoDark() {
    const { t } = useLanguage();
  ...
  return (
    <div className="max-w-full mx-auto space-y-6 animate-in slide-in-from-bottom-4 duration-500 fade-in pb-20 md:pb-0">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-[20.104264919475px] md:text-[26.7902365993px] font-bold text-white">{t(`Αναφορά Προβλήματος`, `Report an Issue`)}</h1>
          <p className="text-white font-medium text-[13.551608211075px] md:text-[16.25212883329px] mt-1">{t(`Βοηθήστε μας να κρατήσουμε την κοινότητα ασφαλή`, `Help us keep the community safe`)}</p>
        </div>
        <button onClick={() => navigate(-1)} className="text-white hover:text-white transition-colors rounded-full p-2 bg-gray-800 border-gray-700 text-white border border-gray-700 shadow-sm">
          <X className="w-5 h-5" />
        </button>
      </div>
      ...
      {step === 1 ? (
        <div className="bg-gray-800 border-gray-700 text-white rounded-xl shadow-sm border border-gray-700 overflow-hidden">
          <div className="p-4 md:p-6 bg-red-50 border-b border-red-100 flex items-start gap-4">
            ...
            <div>
              <h3 className="font-bold text-white text-[16.75971px]">{t(`Αναφορά Ζητήματος Ασφαλείας`, `Report a Safety Concern`)}</h3>
              <p className="text-[14.535px] text--400 mt-1 leading-relaxed">{t(`Η αναφορά σας θα εξεταστεί από την ομάδα μας εντός 24 ωρών.`, `Your report will be reviewed by our team within 24 hours.`)}</p>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="p-4 md:p-6 space-y-5">
            ...
            {/* Severity Level */}
            <div>
              <label className="block text-[10.90125px] font-bold text-white tracking-wider mb-2">{t(`Σοβαρότητα`, `Severity`)}</label>
            ...
            <div className="flex items-center gap-2 justify-center text-[12.5px] text-white font-medium">
              <Clock className="w-3.5 h-3.5" />
              <span>{t(`Αναμενόμενος χρόνος απόκρισης: <24 ώρες`, `Expected response time: <24 hours`)}</span>
            </div>

            <button type="submit" className="w-full bg-gray-800 shadow-[0_8px_30px_rgb(0,0,0,0.5)] border border-gray-700 text-white py-2.5 rounded-full text-[12.15px] font-bold shadow-sm hover:bg-black transition-colors  tracking-wider">{t(`Υποβολή Αναφοράς`, `Submit Report`)}</button>
          </form>
        </div>
      ) : (
        <div className="bg-gray-800 border-gray-700 text-white rounded-xl shadow-sm border border-gray-700 p-8 text-center">
          ...
          <p className="text-[18px] text-white max-w-md mx-auto mb-6">{t(`Ευχαριστούμε. Θα εξετάσουμε την αναφορά σας και θα σας ενημερώσουμε.`, `Thank you. We'll review your report and get back to you.`)}</p>
          <button 
            onClick={() => navigate('/')}
            className="bg-gray-800 border border-gray-700 text-white px-5 py-2.5 rounded-full text-[13.5px] font-bold hover:bg-gray-200 transition-colors tracking-wider"
