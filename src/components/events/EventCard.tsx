    ...
    >
        ...
        <div className="absolute bottom-3 left-3 right-3 flex items-end justify-between z-10 gap-2">
          <div className="flex flex-nowrap overflow-hidden gap-1.5 flex-1 w-full max-w-[65%]">
            {(event.tags ?? []).slice(0, 2).map((tag: string) => (
              <span
                key={tag}
                className="bg-black/50 backdrop-blur-md text-white/95 border border-white/20 px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-md text-[10px] font-semibold capitalize tracking-wide shadow-sm whitespace-nowrap truncate max-w-[100px]"
              >
                {tag}
              </span>
            ))}
          </div>
          <div className="bg-white/95 backdrop-blur-sm px-2.5 py-1 sm:px-3 sm:py-1.5 rounded-lg shadow-lg font-black text-[#111827] text-[11px] sm:text-xs tracking-tight flex flex-col items-end shrink-0 border border-white/60">
            {event.isPaid ? `€${event.price}` : t("event_card.free", "Free")}
            {event.isPaid && event.groupDiscount && (
              <span className="text-[9px] text-[#0E8B8D] leading-none block mt-0.5 font-bold">
      ...
      <div className="p-4 flex flex-col flex-1">
        <div className="flex items-start justify-between gap-3 mb-2">
          ...
          {currentUser && (
            <div className="flex flex-col items-end shrink-0">
              <span className="text-[12px] font-black text-[#18D8DB] leading-none">
                {Math.min(
                  99,
                  45 +
                    (event.tags ?? []).filter((t) =>
              ...
              <span className="text-[9px] font-semibold text-gray-500 capitalize tracking-wide">
                {t("Ταίριασμα", "Match")}
              </span>
            </div>
          )}
        ...
        <div className="flex flex-col gap-2 mb-4">
          <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-gray-600 text-[12px] font-medium">
            <div className="flex items-center bg-gray-50 px-2 py-1 rounded-md">
              <Clock className="w-3.5 h-3.5 mr-1.5 text-indigo-500 shrink-0" />
          ...
          <div className="flex items-center text-gray-600 text-[12px] font-medium mt-2">
          ...
          {userGroup && (
            <button
               ...
               className="mt-3 w-full bg-indigo-50 hover:bg-indigo-100 text-indigo-700 py-2 rounded-xl text-xs font-bold transition-colors flex items-center justify-center gap-2 border border-indigo-100"
            >
          ...
          <div className="flex items-center justify-between mb-4 bg-gray-50/50 rounded-xl p-2.5 border border-gray-100 hover:bg-gray-50 transition-colors">
            <div className="flex items-center gap-2.5">
              ...
              <div className="flex flex-col">
                <span className="text-[10px] text-gray-500 font-semibold mb-0.5 capitalize">
                  {t("event_card.organizer", "Organizer")}
                </span>
                <button
                  className="text-xs font-bold text-gray-900 text-left hover:text-indigo-600 transition-colors line-clamp-1"
                  onClick={(e) => {
                    e.stopPropagation();
        ...
        <div className="mb-3">
          ...
          >
            <span className="text-[11px] font-bold text-gray-900 flex items-center gap-1.5 capitalize tracking-tight">
          ...
          {showSafetyPanel && (
            ...
            >
              ...
              <div className="space-y-1.5">
                <p className="text-[11px] font-semibold text-gray-600 tracking-tight capitalize">
                ...
                <div className="grid grid-cols-2 gap-1.5">
                  {[
                    ...
                    <button
                      ...
                      className={`text-[9px] font-bold py-1.5 px-2 rounded-lg border transition-colors ${arrivalStatus === status ? "bg-indigo-600 text-white border-indigo-600" : "bg-white text-gray-600 border-gray-200 hover:border-indigo-300"}`}
                    >
                      {status}
                    </button>
                  ))}
              ...
              <div className="space-y-1.5">
                <div className="flex gap-1.5">
                  <button
                    ...
                    className="flex-1 text-[10px] font-bold py-1.5 px-2 bg-white text-indigo-700 border border-indigo-100 rounded-lg hover:bg-indigo-50 flex items-center justify-center gap-1"
                  >
                  ...
                  <button
                    ...
                    className="flex-1 text-[10px] font-bold py-1.5 px-2 bg-emerald-50 text-emerald-700 border border-emerald-100 rounded-lg hover:bg-emerald-100 flex items-center justify-center gap-1"
                  >
                ...
                {showEtaPrompt && (
                  <div className="flex gap-2 items-center bg-white p-2 rounded border border-gray-200 mt-1.5 shadow-sm">
                    <input
                      ...
                      className="flex-1 text-[11px] font-medium p-1 border-b border-gray-200 focus:border-indigo-600 outline-none"
                    ...
                    <button
                      ...
                      className="text-[10px] font-bold bg-indigo-600 text-white px-3 py-1 rounded"
                    >
                      {t("Αποστολή", "Send")}
                    </button>
                  </div>
              ...
              <div className="pt-1.5">
                <button
                  ...
                  className={`w-full flex items-center justify-between px-3 py-2 border rounded-lg transition-colors text-left shadow-sm ${isSharingLocation ? "bg-indigo-50 border-indigo-200 hover:bg-indigo-100" : "bg-white hover:bg-gray-50 border-gray-200"}`}
                >
                  <div>
                    <p
                      className={`text-[11px] font-bold ${isSharingLocation ? "text-indigo-700" : "text-[#111827]"}`}
                    >
                      {isSharingLocation
                    ...
                    <p
                      className={`text-[9px] mt-0.5 ${isSharingLocation ? "text-indigo-600/80" : "text-gray-500"}`}
                    >
                      {isSharingLocation
                        ? t(
                  ...
                  <MapPin
                    className={`w-3.5 h-3.5 ${isSharingLocation ? "text-indigo-600" : "text-gray-400"}`}
                  />
                </button>
              </div>
            </div>
