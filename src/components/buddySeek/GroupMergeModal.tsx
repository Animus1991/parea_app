    ...
    <div className="fixed inset-0 z-[85] flex items-end sm:items-center justify-center p-0 sm:p-4">
      ...
      <div className="relative w-full sm:max-w-md rounded-t-3xl sm:rounded-3xl bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 shadow-2xl max-h-[90vh] overflow-y-auto">
        <div className="p-5 border-b border-gray-100 dark:border-gray-800">
          ...
          <p className="text-[12px] text-gray-500 mt-1">{event.title}</p>
        </div>

        <div className="p-5 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div className="rounded-2xl border p-3 dark:border-gray-700">
              <p className="text-[10px] font-bold uppercase text-gray-500">{t('Η ομάδα σας', 'Your group')}</p>
            ...
            <div className="rounded-2xl border p-3 dark:border-gray-700">
              <p className="text-[10px] font-bold uppercase text-gray-500">{t('Άλλη ομάδα', 'Other group')}</p>
              <p className="text-lg font-black flex items-center gap-1 mt-1">
                <Users className="w-4 h-4" />
          ...
          {pending ? (
            <div className="rounded-2xl bg-amber-50 dark:bg-amber-950/30 border border-amber-200 p-3">
              ...
              <button
                ...
                className="mt-2 w-full min-h-10 rounded-xl bg-emerald-600 text-white text-[12px] font-bold"
              >
            ...
            <>
              <fieldset className="space-y-2">
                <legend className="text-[12px] font-bold text-gray-900 dark:text-white">
              ...
              <textarea
                ...
                className="w-full rounded-2xl border px-3 py-2 text-[12px] min-h-[72px] dark:bg-gray-800 dark:border-gray-700"
              />
            </>
          )}
        ...
        <div className="p-5 border-t flex gap-2 sticky bottom-0 bg-white dark:bg-gray-900">
          <button
            type="button"
            onClick={onClose}
            className="flex-1 min-h-11 rounded-2xl border text-[12px] font-bold text-gray-700 dark:text-gray-300"
          >
            {t('Ακύρωση', 'Cancel')}
          </button>
          {!pending && (
            <button
              type="button"
              onClick={send}
              className="flex-[2] min-h-11 rounded-2xl bg-cyan-600 text-white text-[12px] font-bold"
            >
